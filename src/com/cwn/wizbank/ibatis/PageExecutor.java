...
public class PageExecutor implements Executor {
	...
	private <E> void doCache(MappedStatement ms, Page<E> result, Object parameter,
			RowBounds rowBounds) {
		logger.debug(" page Executor execute");
/*		final Cache cache = ms.getCache();
		if (executor.getClass().isAssignableFrom(CachingExecutor.class)
				&& cache != null) {
			BoundSql boundSql = ms.getBoundSql(parameter);
			final CacheKey cacheKey = createCacheKey(ms, parameter, rowBounds,
					boundSql);
