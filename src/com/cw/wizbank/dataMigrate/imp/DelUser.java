...
public class DelUser extends Del {
	...
	public DelUser(Connection conn, WizbiniLoader wizbiniLoader, String logDir, String file_path) {
		con = conn;
		...
		fieldLabel = ImportTemplate.delTitle.toArray(new String[ImportTemplate.delTitle.size()]);
	 ...
	 * @param prof 日志
	 */
	private void deleteUser(UserBean user,loginProfile prof){
		try {
			regUserService.del(user.getUser_id(),1,prof.getRoot_ent_id(),user.getUser_id());
			logSuccess(LangLabel.getValue(prof.cur_lan,"line_number" )+user.lineno+"  "+LangLabel.getValue( prof.cur_lan,"lab_user_id")+"["+user.getUser_id()+"]"+LangLabel.getValue( prof.cur_lan,"del_success")+".");
		}catch (Exception e){
			e.printStackTrace();
	...
	public void importData(Vector userVec, loginProfile prof, ImportStatus importStatus,boolean clear) throws Exception {
		...
		//循环删除
		for(int i= 0 ; i<userVec.size() ; i++){
			UserBean crb = (UserBean)userVec.get(i);
			//删除用户数据
			deleteUser(crb,prof);
		}
		if(clear){
