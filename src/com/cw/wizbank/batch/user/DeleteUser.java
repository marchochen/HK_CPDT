package com.cw.wizbank.batch.user;
...
public class DeleteUser {
	public static void deleteUser(Connection con, long usr_ent_id) throws SQLException, qdbException {
		...
		if (usr.usr_status.equalsIgnoreCase(dbRegUser.USR_STATUS_SYS)) {
			CommonLog.error("不允许删除系统帐号");
			return;
		}

		insDeletedUserInfo(con, usr, cur_time);

		// 个人设定
		upd(con, " delete from psnBiography where pbg_ent_id = ?", usr.usr_ent_id);
		upd(con, " delete from psnPreference where pfr_ent_id = ?", usr.usr_ent_id);

		// 目录
		upd(con, " delete from aeCatalogAccess where cac_ent_id = ?", usr.usr_ent_id);
		upd(con, " delete from aeTreeNodeSubscribe where tsb_ent_id = ?", usr.usr_ent_id);

		// 报表
		upd(con, " delete from ReportSpec where rsp_ent_id = ?", usr.usr_ent_id);

		// 邮件和站内信

		upd(con, " delete from webMsgReadHistory where wmrh_wmsg_id in (select wmsg_id from webMessage where wmsg_send_ent_id = ? or wmsg_create_ent_id = ? or wmsg_rec_ent_id = ?)", usr.usr_ent_id, usr.usr_ent_id, usr.usr_ent_id);
		upd(con, " delete from webMessage where wmsg_send_ent_id = ? or wmsg_create_ent_id = ? or wmsg_rec_ent_id = ? ", usr.usr_ent_id, usr.usr_ent_id, usr.usr_ent_id);
		upd(con, " delete from emailMsgRecHistory where emrh_emsg_id in (select emsg_id from emailMessage where emsg_rec_ent_ids = ? or emsg_send_ent_id = ?)", usr.usr_ent_id+"", usr.usr_ent_id);
		upd(con, " delete from emailMessage where emsg_rec_ent_ids = ? or emsg_send_ent_id = ?", usr.usr_ent_id+"", usr.usr_ent_id);

		// 培训中心
		upd(con, " delete from tcTrainingCenterOfficer where tco_usr_ent_id = ?", usr.usr_ent_id);
		upd(con, " delete from tcTrainingCenterTargetEntity where tce_ent_id = ?", usr.usr_ent_id);

		// 问答
		upd(con, " delete from sns_doing where s_doi_target_id in (select ans_id from knowAnswer where ans_create_ent_id = ?) and Lower(s_doi_module) = 'answer' ", usr.usr_ent_id);
		upd(con, " delete from knowVoteDetail where kvd_ans_id in (select ans_id from knowAnswer where ans_create_ent_id = ?) ", usr.usr_ent_id);
		upd(con, " delete from knowAnswer where ans_create_ent_id = ? ", usr.usr_ent_id);
		// 删除用户所提过的问题
		List<Long> que_id_list = KnowQuestionDAO.getQueIdListByUsrEntId(con, usr.usr_ent_id);
		for(long que_id : que_id_list){
			Know.delKownQuestion(con, que_id);
		}

		// 积分
		upd(con, " delete from userCreditsDetailLog where ucl_usr_ent_id = ?", usr.usr_ent_id);
		upd(con, " delete from userCreditsDetail where ucd_ent_id = ?", usr.usr_ent_id);
		upd(con, " delete from userCredits where uct_ent_id = ?", usr.usr_ent_id);

		// 目标学员
		upd(con, " delete from RegUserSkillSet where uss_ent_id = ?", usr.usr_ent_id);
		
		//知识中心
		upd(con, " delete from kb_item_view where kiv_usr_ent_id = ?", usr.usr_ent_id);

		upd(con, " delete from usrPwdResetHis where prh_ent_id = ?", usr.usr_ent_id);
		upd(con, " delete from UsrRoleTargetEntity where rte_usr_ent_id = ?", usr.usr_ent_id);
		upd(con, " delete from aeNotifyEntity where nen_ent_id = ?", usr.usr_ent_id);
		upd(con, " delete from acEntityRole where erl_ent_id = ?", usr.usr_ent_id);
		upd(con, " delete from enrolment where enr_ent_id = ?", usr.usr_ent_id);
		upd(con, " delete from resourcePermission where rpm_ent_id = ?", usr.usr_ent_id);

		// 删除报名学习记录
		upd(con, " delete from aeLearningSoln where lsn_ent_id =?", usr.usr_ent_id);
		upd(con, " delete from IndustryCode where idc_ent_id =?", usr.usr_ent_id);
		upd(con, " delete from cmAssessmentUnit where asu_ent_id =?", usr.usr_ent_id);
		upd(con, " delete from cmAssessment where asm_ent_id =?", usr.usr_ent_id);
		upd(con, " delete from ObjectiveAccess where oac_ent_id =?", usr.usr_ent_id);
		upd(con, " delete from cfCertification where cfn_ent_id =?", usr.usr_ent_id);
		upd(con, " delete from AiccPath where acp_ent_id =?", usr.usr_ent_id);
		upd(con, " delete from aeItemcomments where ict_ent_id =?", usr.usr_ent_id);
		upd(con, " delete from accomplishment where apm_ent_id =?", usr.usr_ent_id);
		upd(con, " delete from measurementevaluation where mtv_ent_id =?", usr.usr_ent_id);
		upd(con, " delete from moduleevaluationhistory where mvh_ent_id =?", usr.usr_ent_id);
		upd(con, " delete from progressattemptsaveanswer where psa_tkh_id in (select app_tkh_id from aeApplication where app_ent_id =?)", usr.usr_ent_id);
		upd(con, " delete from progressattemptsave where pas_tkh_id in (select app_tkh_id from aeApplication where app_ent_id =?)", usr.usr_ent_id);
		upd(con, " delete from progressattachment where pat_tkh_id in (select app_tkh_id from aeApplication where app_ent_id =?)", usr.usr_ent_id);
		upd(con, " delete from progressattachment where pat_prg_usr_id in (select pgr_usr_id from progress where pgr_usr_id =?)", usr.usr_id);
		upd(con, " delete from ProgressAttempt where atm_tkh_id in (select app_tkh_id from aeApplication where app_ent_id =?)", usr.usr_ent_id);
		upd(con, " delete from ProgressAttempt where atm_pgr_usr_id in (select pgr_usr_id from progress where pgr_usr_id =?)", usr.usr_id);
		upd(con, " delete from progress where pgr_usr_id =?", usr.usr_id);
		upd(con, " delete from progress where pgr_tkh_id in (select app_tkh_id from aeApplication where app_ent_id =?)", usr.usr_ent_id);
		upd(con, " delete from moduleevaluation where mov_ent_id = ?", usr.usr_ent_id);
		upd(con, " delete from courseevaluation where cov_ent_id = ?", usr.usr_ent_id);
		upd(con, " delete from aeappnOpenItem where aoi_app_id in (select app_id from aeApplication where app_ent_id =?)", usr.usr_ent_id);
		upd(con, " delete from aeAppnTargetEntity where ate_usr_ent_id = ?", usr.usr_ent_id);
		upd(con, " delete from TrackingHistory where tkh_usr_ent_id = ?", usr.usr_ent_id);
		upd(con, " delete from aeAppnCommHistory where ach_app_id in (select app_id from aeApplication where app_ent_id =?)", usr.usr_ent_id);
		upd(con, " delete from aeAppnApprovalList where aal_usr_ent_id = ?", usr.usr_ent_id);
		upd(con, " delete from aeAppnApprovalList where aal_app_ent_id = ?", usr.usr_ent_id);
		upd(con, " delete from aeAppnApprovalList where aal_action_taker_usr_ent_id = ?", usr.usr_ent_id);
		upd(con, " delete from aeAppnapprovallist where aal_app_id in (select app_id from aeApplication where app_ent_id =?)", usr.usr_ent_id);
		upd(con, " delete from aeappnactnhistory where aah_app_id in (select app_id from aeApplication where app_ent_id =?)", usr.usr_ent_id);
		upd(con, " delete from aeAttendance where att_app_id in (select app_id from aeApplication where app_ent_id =?)", usr.usr_ent_id);
		upd(con, " delete from aeApplication where app_ent_id = ?", usr.usr_ent_id);
		upd(con, " delete from InstructorComment where itc_ent_id = ? or itc_iti_ent_id = ? ", usr.usr_ent_id, usr.usr_ent_id);
		upd(con, " delete from InstructorCos where ics_iti_ent_id = ? ", usr.usr_ent_id);

		// 用户组、直属上司
		upd(con, " delete from EntityLoginHistory where elh_ent_id = ? ", usr.usr_ent_id);
		upd(con, " delete from EntityRelationHistory where erh_child_ent_id = ? ", usr.usr_ent_id);
		upd(con, " delete from EntityRelation where ern_child_ent_id = ? ", usr.usr_ent_id);
		upd(con, " delete from SuperviseTargetEntity where spt_source_usr_ent_id = ? or spt_target_ent_id = ? ", usr.usr_ent_id, usr.usr_ent_id);
		upd(con, " delete from aeAccount where acn_ent_id = ? ", usr.usr_ent_id);
		upd(con, " delete from aeItemrating where irt_ent_id = ? ", usr.usr_ent_id);
		upd(con, " delete from aeItemLessonInstructor where ili_usr_ent_id = ? ", usr.usr_ent_id);
		upd(con, " delete from aeItemaccess where iac_ent_id = ? ", usr.usr_ent_id);

		//	社区
		upd(con, " delete from sns_collect where s_clt_uid = ? ", usr.usr_ent_id);
		upd(con, " delete from sns_attention where s_att_source_uid = ? or s_att_target_uid = ? ", usr.usr_ent_id, usr.usr_ent_id);
		upd(con, " delete from sns_share where s_sha_uid = ? ", usr.usr_ent_id);
		upd(con, " delete from sns_setting where s_set_uid = ? ", usr.usr_ent_id);
		upd(con, " delete from sns_valuation_log where s_vtl_uid = ? ", usr.usr_ent_id);
		upd(con, " delete from sns_count where s_cnt_target_id in (select s_cmt_id from sns_comment where s_cmt_uid = ?) and s_cnt_is_comment = 1 ", usr.usr_ent_id);
		upd(con, " delete from sns_count where s_cnt_target_id in (select s_doi_id from sns_doing where s_doi_uid = ?) and Lower(s_cnt_module) = 'doing' and s_cnt_is_comment = 0 ", usr.usr_ent_id);
		upd(con, " delete from sns_comment where s_cmt_uid = ? ", usr.usr_ent_id);
		upd(con, " delete from sns_doing where s_doi_uid = ? ", usr.usr_ent_id);
		upd(con, " delete from sns_group_member where s_gpm_grp_id in (select s_grp_id from sns_group where s_grp_uid = ?) or s_gpm_usr_id = ? ", usr.usr_ent_id, usr.usr_ent_id);
		upd(con, " delete from sns_group where s_grp_uid = ? ", usr.usr_ent_id);
		
		// 供应商
		upd(con, " delete from supplierComment where scm_ent_id = ? or scm_spl_id in (select spl_id from supplier where spl_create_usr_id = ?) ", usr.usr_ent_id, usr.usr_id);
		upd(con, " delete from supplierCooperationExperience where sce_spl_id in (select spl_id from supplier where spl_create_usr_id = ?) ", usr.usr_id);
		upd(con, " delete from supplierMainCourse where smc_spl_id in (select spl_id from supplier where spl_create_usr_id = ?) ", usr.usr_id);
		upd(con, " delete from supplier where spl_create_usr_id = ? ", usr.usr_id);
		
		updUsrInfo(con, usr.usr_id, usr.usr_ent_id);
		
		// 用户信息
		upd(con, " delete from UserPositionRelation where upr_usr_ent_id = ? ", usr.usr_ent_id);
		upd(con, " delete from UserClassification where ucf_ent_id = ?", usr.usr_ent_id);
		upd(con, " delete from RegUserExtension where urx_usr_ent_id = ?", usr.usr_ent_id);
		upd(con, " delete from RegUser where usr_ent_id = ?", usr.usr_ent_id);
		upd(con, " delete from Entity where ent_id = ?", usr.usr_ent_id);
		
		//用户的token
		upd(con," delete from APIToken where atk_usr_ent_id = ? ",usr.usr_ent_id);
	}
	...
	private static void insDeletedUserInfo(Connection con, dbRegUser usr, Timestamp cur_time) throws SQLException {
		QueryRunner run = new QueryRunner();
		run.update(con, " insert into DeletedUserInfo (dui_usr_ent_id, dui_usr_ste_usr_id, dui_usr_display_bil, dui_delete_timestamp) values (?, ?, ?, ?)", usr.usr_ent_id, usr.usr_ste_usr_id, usr.usr_display_bil, cur_time);
	}
