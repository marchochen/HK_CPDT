...
public class DateTimeValidator extends DMTypeValidator implements Serializable {
	 ...
	 * 
	 * @param iInclude
	 *            Indicates if subseconds should be tested for in this
	 *            validator.
	 */
				...
				if (idx != iValue.length()) {
					if (iValue.charAt(idx) == '-') {
						...
						try {
							...
							// Create a calendar for the indicated year and
							// month
							...
							if (day < 1 || day > days) {
								...
								// Valid day, but need to check if we are at the
								// end
								idx += 2;
								if (idx == iValue.length()) {
									done = true;
								} else {
									// Make sure the next section is describing
									// a time
						...
						if (mIncludeSubSecs) {
							if (idx != iValue.length()) {
								if (iValue.charAt(idx) == '.') {
									...
									// Found start of subseconds, try to find
									// the end
									...
									if (lengthCounter == 0 || lengthCounter > 2) {
										...
										try {
											...
											// if exception
											// is thrown, then set appropriate
											// error code.
