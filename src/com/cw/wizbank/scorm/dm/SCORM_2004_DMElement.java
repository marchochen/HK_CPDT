...
import com.cw.wizbank.scorm.dm.Children;
import com.cw.wizbank.scorm.dm.Count;
import com.cw.wizbank.scorm.dm.DMDelimiter;
import com.cw.wizbank.scorm.dm.DMDelimiterDescriptor;
import com.cw.wizbank.scorm.dm.DMElement;
import com.cw.wizbank.scorm.dm.DMElementDescriptor;
...
import com.cw.wizbank.scorm.dm.DataModel;
import com.cw.wizbank.scorm.dm.RequestDelimiter;
import com.cw.wizbank.scorm.dm.RequestToken;
import com.cw.wizbank.scorm.dm.datatypes.LangStringValidator;
import com.cw.wizbank.scorm.dm.datatypes.SPMRangeValidator;
import com.cw.wizbank.scorm.dm.datatypes.VocabularyValidator;
import com.cw.wizbank.scorm.util.LogMessage;
import com.cw.wizbank.scorm.util.MessageType;

/**
 * SCORM_2004_DMElement
 */
public class SCORM_2004_DMElement extends DMElement implements Serializable {
	 ...
	 * 
	 * @param iDescription
	 *            Identies which data characteristics of the data model element.
	 * 
	 * @param iParent
	 *            Describes the parent of this data model element.
	 * 
	 * @param iDM
	 *            Describes the data model containing this data model element.
	 */
	public SCORM_2004_DMElement(DMElementDescriptor iDescription, DMElement iParent, DataModel iDM) {
		...
		// Check if this element is managing records -- is it an array
		// container?
		...
		if (mDescription.mChildren != null) {
			...
			for (int i = 0; i < mDescription.mChildren.size(); i++) {
				DMElementDescriptor desc = (DMElementDescriptor) mDescription.mChildren.elementAt(i);
			...
			if (mDescription.mDelimiters != null) {
				...
				for (int i = 0; i < mDescription.mDelimiters.size(); i++) {
					DMDelimiterDescriptor desc = (DMDelimiterDescriptor) mDescription.mDelimiters.elementAt(i);
	...
	public int equals(RequestToken iValue, boolean iValidate) {
		...
		if (mValue != null && iValue != null) {
			...
			if (result == DMErrorCodes.TYPE_MISMATCH) {
				// An invalid value cannot be equal to any valid (set) data
				// model
				// element.
				equal = false;
			}
			...
			if (equal) {
				...
				// If delimiters are defined on this data model element, make
				// sure
				...
				if (mDelimiters != null) {
					...
					for (; i < iValue.getDelimiterCount() && equal; i++) {
						...
						// Check if this element includes the specified
						// delimiter
						for (int j = 0; j < mDelimiters.size() && equal; j++) {
							...
							if (toCheck.mDescription.mName.equals(del.getName())) {
								...
								// delimter
								boolean alreadyChecked = ((Boolean) checked.elementAt(j)).booleanValue();
								...
								if (!alreadyChecked) {
									...
									// Check if the value is the delimiter's
									// default
									if (toCheck.mValue == null) {
										if (toCheck.mDescription.mDefault != null) {
											// Compare the provided value with
											// the default
											if (toCheck.mDescription.mValidator == null) {
												equal = equal && toCheck.mDescription.mDefault.equals(del.getValue());
											} else {
												equal = equal && toCheck.mDescription.mValidator.compare(toCheck.mDescription.mDefault, del.getValue(), mDelimiters);
											}
										} else {
											equal = false;
										}
										...
										if (toCheck.mDescription.mValidator == null) {
											...
											// test the SPM
											if (!equal) {
												equal = toCheck.mDescription.mValidator.compare(toCheck.mDescription.mValidator.trunc(toCheck.mValue), del.getValue(), mDelimiters);
											}
										}
									}
								}
					...
					if (equal) {
						// Make sure any delimters not included in the request
						// are
						// equal to their defaults
						for (int j = 0; j < mDelimiters.size() && equal; j++) {
							boolean check = !((Boolean) checked.elementAt(j)).booleanValue();
				...
				// Only compare the data model values if its delimiters where
				// equal
				if (equal) {
					...
					// If no comparison method is defined, just do a string
					// compare
					if (mDescription.mValidator == null) {
						equal = compareWith.equals(mValue);
					} else {
						equal = mDescription.mValidator.compare(compareWith, mValue, mDelimiters);

						// If the first compare doesn't work test the SPM
						if (!equal) {
							equal = mDescription.mValidator.compare(compareWith, mDescription.mValidator.trunc(mValue), mDelimiters);
						}
					}
				}
			}
					...
					if (mRecords != null) {
						...
						if (mChildrenBindings != null) {
							...
							if (idx != -1) {
								...
								DMElement element = (DMElement) mChildren.get(tok.getValue());
								...
								} else // SetValue()
								{
									...
									// Enforce data model dependencies, if they
									// exist
									if (desc.mDependentOn != null) {
										...
										for (int i = 0; i < desc.mDependentOn.size(); i++) {
											...
											// Ensure the dependent element is
											// initialized
											DMElement e = (DMElement) mChildren.get(check);

											if (e != null) {
												if (!e.isInitialized()) {
													// Dependend element is not
													// initalized
				...
				} else // SetValue()
				{
					...
					if (idx >= mRecords.size()) {
						...
						if (idx == mRecords.size() && !(mDescription.mMaximum && mRecords.size() == mDescription.mSPM)) {
							...
							// We can assume that we only need to look at the
							// next
							// token because the dependency will be defined on
							// that
							// token's element
							...
							if (lookAt != null) {
								...
								// Look for this element in the children of the
								// record
								// NOTE: This implementation assumes there are
								// no
								// nested arrays -- SCORM 2004 does not require
								// them
								for (int i = 0; i < mDescription.mChildren.size(); i++) {
								...
								if (!found) {
									// The request child element does not exist,
									// so
									// there is no reason to create the new
									// record
									result = DMErrorCodes.UNDEFINED_ELEMENT;
								...
								// No next token, so there is no reason to
								// create the
								// new record
								result = DMErrorCodes.UNDEFINED_ELEMENT;
							...
							if (ok) {
								// Only create the new record if the array is
								// not
								...
								if (isWriteable || ioRequest.isAdminRequest()) {
									...
									if (mRecords.size() > mDescription.mSPM) {
										// Collection SPM Exceeded, create
										// warning.
										String dn = getDotNotation(mDM);
										String warn = "Collection SPM exceeded";
									}
								} else {
									// Don't create this record when trying to
									// set a
									// read-only child
							...
							if (mRecords.size() == mDescription.mSPM && mDescription.mMaximum) {
								// Exceeds absolute maximum -- nothing was
								// created
							...
							if (!setAlready) {
								...
								if (toSet.mDescription.mDefault != null) {
									if (!toSet.mDescription.mDefault.equals(del.getValue())) {
										...
										// Check to see if we need to trucate
										// the value
...
//					mValue = InteractionTrunc.trunc(mValue, ((InteractionValidator) mDescription.mValidator).getType());
			...
			// the SPM and datatype of the correct_responses and
			// learner_response
			if (mDescription.mBinding.equals("type")) {
				...
				if (!mInitialized || !mValue.equals(oldValue)) {
					...
					for (int j = 0; j < desc.mChildren.size(); j++) {
						...
						// and set the ith index of the mChildren array when
						// finished
...
/*						if (curChild.mBinding.equals("correct_responses")) {
							...
							// Switch the SPM and data type of the
							// correct_responses
			...
			for (; i < iValue.getDelimiterCount() && found; i++) {
				...
				for (int j = 0; j < mDelimiters.size() && !found; j++) {
					...
					if (toCheck.mDescription.mName.equals(trim)) {
						...
						if (!alreadyChecked) {
							// If a known delimiter has whitespace, flag the
							// error
							if (ws) {
								...
								if (toCheck.mDescription.mValidator != null) {
									...
									// If any of the delimiters are invalid, we
									// are done
									if (result != DMErrorCodes.NO_ERROR) {
										break;
									}
   ...
   *//**
	 ...
	 * 
	 * @param ioChild
	 *            The <code>DMElementDescriptor</code> for the interaction
	 ...
	 * // validator. String e = "cmi.interactions.n.learner_response";
	 * 
	 * DMDelimiterDescriptor del = null;
	 * 
	 * if ( mValue.equals("true-false") ) { String [] boolVocab = {"true",
	 * "false"}; ioChild.mValidator = new VocabularyValidator(boolVocab); } else
	 * if ( mValue.equals("choice") ) { ioChild.mValidator = new
	 * InteractionValidator(InteractionValidator.MULTIPLE_CHOICE, e); } else if
	 * ( mValue.equals("fill-in") ) { ioChild.mValueSPM = -2;
	 * 
	 * ioChild.mValidator = new
	 * InteractionValidator(InteractionValidator.FILL_IN, e); } else if (
	 * mValue.equals("long-fill-in") ) { // Add the lang delimiter del = new
	 * DMDelimiterDescriptor("lang", "en", DataModel.SHORT_SPM, new
	 * LangStringValidator()); ioChild.mDelimiters = new Vector();
	 * ioChild.mDelimiters.add(del);
	 * 
	 * ioChild.mValueSPM = -2;
	 * 
	 * ioChild.mValidator = new
	 * InteractionValidator(InteractionValidator.LONG_FILL_IN, e); } else if (
	 * mValue.equals("likert") ) { ioChild.mValueSPM = -2;
	 * 
	 * ioChild.mValidator = new
	 * InteractionValidator(InteractionValidator.LIKERT, e); } else if (
	 * mValue.equals("matching") ) { ioChild.mValueSPM = -2;
	 * 
	 * ioChild.mValidator = new
	 * InteractionValidator(InteractionValidator.MATCHING, e); } else if (
	 * mValue.equals("performance") ) { ioChild.mValueSPM = -2;
	 * 
	 * ioChild.mValidator = new
	 * InteractionValidator(InteractionValidator.PERFORMANCE, false, e); } else
	 * if ( mValue.equals("sequencing") ) { ioChild.mValueSPM = -2;
	 * 
	 * ioChild.mValidator = new
	 * InteractionValidator(InteractionValidator.SEQUENCING, e); } else if (
	 * mValue.equals("numeric") ) { ioChild.mValueSPM = -2;
	 * 
	 * ioChild.mValidator = new
	 * InteractionValidator(InteractionValidator.NUMERIC, e); } else if (
	 * mValue.equals("other") ) { ioChild.mValueSPM = DataModel.LONG_SPM;
	 * ioChild.mValidator = new SPMRangeValidator(DataModel.LONG_SPM); } }
	 *//**
	 * Handles setting an interaction's correct_responses element datatype
	 * and SPM based on the interaction type element.
	 * 
	 * @param ioChild
	 *            The <code>DMElementDescriptor</code> for the interaction
	 *            element whose type is being changed.
	 ...
	 * pattern.mIsUnique = false; pattern.mDelimiters = null; pattern.mValidator
	 * = null;
	 * 
	 * if ( pattern.mBinding.equals("pattern") ) { if (
	 * mValue.equals("true-false") ) { ioChild.mSPM = 1; ioChild.mMaximum =
	 * true;
	 * 
	 * String [] boolVocab = {"true", "false"}; pattern.mValidator = new
	 * VocabularyValidator(boolVocab); } else if ( mValue.equals("choice") ) {
	 * ioChild.mSPM = 10; ioChild.mMaximum = false;
	 * 
	 * pattern.mIsUnique = true; pattern.mValueSPM = -2;
	 * 
	 * pattern.mValidator = new
	 * InteractionValidator(InteractionValidator.MULTIPLE_CHOICE, e); } else if
	 * ( mValue.equals("fill-in") ) { // Add the case_matters delimiter String
	 * [] boolVocab = {"true", "false"}; del = new
	 * DMDelimiterDescriptor("case_matters", "false", new
	 * VocabularyValidator(boolVocab)); pattern.mDelimiters = new Vector();
	 * pattern.mDelimiters.add(del);
	 * 
	 * // Add the order_matters delimiter del = new
	 * DMDelimiterDescriptor("order_matters", "true", new
	 * VocabularyValidator(boolVocab)); pattern.mDelimiters.add(del);
	 * 
	 * pattern.mValueSPM = -2;
	 * 
	 * pattern.mValidator = new
	 * InteractionValidator(InteractionValidator.FILL_IN, e);
	 * 
	 * // Set the spm ioChild.mSPM = 5; ioChild.mMaximum = false; } else if (
	 * mValue.equals("long-fill-in") ) { // Add the case_matters delimiter
	 * String [] boolVocab = {"true", "false"}; del = new
	 * DMDelimiterDescriptor("case_matters", "false", new
	 * VocabularyValidator(boolVocab));
	 * 
	 * pattern.mDelimiters = new Vector(); pattern.mDelimiters.add(del);
	 * 
	 * // Add the lang delimiter del = new DMDelimiterDescriptor("lang", "en",
	 * new LangStringValidator()); pattern.mDelimiters.add(del);
	 * 
	 * pattern.mValueSPM = -2;
	 * 
	 * pattern.mValidator = new
	 * InteractionValidator(InteractionValidator.LONG_FILL_IN, e);
	 * 
	 * // Set the patterns spm ioChild.mSPM = 5; ioChild.mMaximum = false; }
	 * else if ( mValue.equals("likert") ) { ioChild.mSPM = 1; ioChild.mMaximum
	 * = true;
	 * 
	 * pattern.mValueSPM = -2;
	 * 
	 * pattern.mValidator = new
	 * InteractionValidator(InteractionValidator.LIKERT, e); } else if (
	 * mValue.equals("matching") ) { ioChild.mSPM = 5; ioChild.mMaximum = false;
	 * 
	 * pattern.mValueSPM = -2;
	 * 
	 * pattern.mValidator = new
	 * InteractionValidator(InteractionValidator.MATCHING, false, e); } else if
	 * ( mValue.equals("performance") ) { // Add the order_matters delimiter
	 * String [] boolVocab = {"true", "false"}; del = new
	 * DMDelimiterDescriptor("order_matters", "true", new
	 * VocabularyValidator(boolVocab)); pattern.mDelimiters = new Vector();
	 * pattern.mDelimiters.add(del);
	 ...
	 * 
	 * pattern.mValidator = new
	 * InteractionValidator(InteractionValidator.PERFORMANCE, false, e);
	 * 
	 * ioChild.mSPM = 5; ioChild.mMaximum = false; } else if (
	 * mValue.equals("sequencing") ) { ioChild.mSPM = 5; ioChild.mMaximum =
	 * false;
	 * 
	 ...
	 * pattern.mValidator = new
	 * InteractionValidator(InteractionValidator.SEQUENCING, e); } else if (
	 * mValue.equals("numeric") ) { ioChild.mSPM = 1; ioChild.mMaximum = true;
	 * 
	 * pattern.mValueSPM = -2;
	 * 
	 * pattern.mValidator = new
	 * InteractionValidator(InteractionValidator.NUMERIC, e); } else if (
	 * mValue.equals("other") ) { ioChild.mSPM = 1; ioChild.mMaximum = true;
	 * 
	 * pattern.mValueSPM = DataModel.LONG_SPM;
	 * 
	 * pattern.mValidator = new SPMRangeValidator(DataModel.LONG_SPM); } } else
	 * { // Really bad error condition }
	 * 
	 * // Replace existing descriptor ioChild.mChildren.removeElementAt(0);
