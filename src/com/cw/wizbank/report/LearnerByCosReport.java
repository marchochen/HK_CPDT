...
public class LearnerByCosReport extends LearnerReport{
	public static final String RTE_TYPE_LEARNER_BY_COS = "LEARNING_ACTIVITY_BY_COS";
	...
	public String getReportTemplate(Connection con, HttpServletRequest request, loginProfile prof, long usr_ent_id, long rsp_id, long rte_id) throws SQLException, qdbException, cwException, IOException, MalformedURLException, cwSysMessage {
		...
		return super.getReport(con, request, prof, rsp_id, rte_id, xml_prefix, null, null, RTE_TYPE_LEARNER_BY_COS, null);
	 }
...
//        			   vec = ViewLearnerReport.getMyRspCosEnrollLrn(con, prof.usr_ent_id, prof.root_ent_id);
        	   ...
        	   if(!spec_pairs.containsKey("tnd_id") && !spec_pairs.containsKey("itm_id") && v_itm_id <= 0) {
        		   all_cos_ind = true;
        	   }
        	   if(all_cos_ind) {
        		   Vector vec = new Vector();
	...
	public String[] getLearningReport(Connection con, HttpServletRequest request, HttpSession sess, cwPagination page, loginProfile prof, qdbEnv env, long rte_id, String[] spec_name, String[] spec_value, long usr_ent_id, String rsp_title, boolean split_ind, boolean override_appr_usg, WizbiniLoader wizbini) throws SQLException, qdbException, cwSysMessage, cwException, IOException, MalformedURLException {
		...
		for(int i=0; i<reportXML.length; i++) {
			reportXML[i] = super.getReport(con, request, prof, 0, rte_id, xml_prefix, reportXML[i], data.rsp_xml, RTE_TYPE_LEARNER_BY_COS, rsp_title);
		}
		return reportXML;
	}
	...
	public String getReport(
		...
		if(usrVec != null && usrVec.size() > 0){
			...
			cwSQL.insertSimpleTempTable(con, tableName, usrVec, cwSQL.COL_TYPE_LONG);
		...
		for (int i = 0; i < keySize; i++) {
			...
			statisInfo.append(StatisticBean.getSummary(env.INI_DIR_UPLOAD_TMP,wizbini.cfgSysSetupadv.getFileUpload().getTmpDir().getUrl(), prof.cur_lan, sb, statusVec,1,v_itm_id <= 0));
			for (int j = 1; j < size; j++) {
				...
				if (d.usr_ent_id!=0 && d.att_ats_id!=0 && d.att_create_timestamp!=null) {
					if (app != null && app.app_usr_prof_xml != null) {
						...
						if (profXML == null) {
							...
							applicant.get(con);
							profXML = applicant.getUserShortXML(con, false, false).toString();
				...
				String group_name = "";
				 if(usr_group_hash != null && usr_group_hash.get(d.usr_ent_id) != null){
					 group_name = full_path.getFullPath(con, (Long) usr_group_hash.get(d.usr_ent_id));
				 }
				recordXML.append("<group>").append(cwUtils.esc4XML(group_name))
				  		 .append("</group>");
				recordXML.append("</group_list>");
				recordXML.append("<grade_list>");
				String grade_name = "";
				 if(usr_grade_hash != null && usr_grade_hash.get(d.usr_ent_id) != null){
					 grade_name = (String)usr_grade_hash.get(d.usr_ent_id);
				 }
				recordXML.append("<group>").append(cwUtils.esc4XML(grade_name))
				  		 .append("</group>");
				recordXML.append("</grade_list>");
		...
		if(v_itm_id <=0 ){
			result.append(StatisticBean.getSummary(env.INI_DIR_UPLOAD_TMP,wizbini.cfgSysSetupadv.getFileUpload().getTmpDir().getUrl(), prof.cur_lan, reportSumm,statusVec,2,v_itm_id <= 0));
		}
			...
			for (int groupIndex = 0; groupIndex < groupVec.size(); groupIndex++) {
				...
				t_code = sb.t_code;
				t_title = sb.t_title;
				result.append("<item code=\"").append(cwUtils.esc4XML(t_code)).append("\" id=\"").append(groupKey).append("\" >");
