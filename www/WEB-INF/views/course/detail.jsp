...
$(function(){
	...
	$.getJSON(detailUrl, function(result) {
 	//	try{
			...
			var itm_retake_ind = item.itm_retake_ind;
 			//定义模板
 			$.templates({
				...
				btnTemplate : '<input type="button" onclick="{{>href}}" value="{{>name}}" class="btn wzb-btn-yellow" style="{{>style}}"></input>',
				...
				moduleWarnField : '<div class="div-warn" style="color:red;"></div>',
			});
			//课程详情
			...
			if(isOffline){
				...
				//}
				navs.push('detail_course_detail');
				navs.push('detail_class_list');
				if(hasCPDFunction){
				...
				if(isClass){
					...
					navs.push('detail_course_notice');
				}
			...
			var schar_itm_desc= item.itm_desc ? item.itm_desc : "";
            var schar_str="";  
            var schar_len=0;  
            var schar;  
            var re = /[^\u4e00-\u9fa5]/;  
            for(var i=0;schar=schar_itm_desc.charAt(i);i++){  
                schar_str+=schar;  
			...
			var data = {
				...
				appnTimeValide : !result.appnTimeValide && item.itm_create_run_ind == 0 ? true : false,
			...
			if(data.btn === 2 || data.btn === 5 || data.btn === 6){
				...
				//var btnContent = "<span style='float:right'>"+Wzb.getButton(data, result.userEntId)+"</span>";
			...
			if(tkhId == undefined || tkhId == '' || tkhId == 0){
...
/**********************网上课程****************************************************************************/
			...
			if(!appStatus || 'Rejected'==appStatus || 'Withdrawn'==appStatus){//还没报名
				switch(result.btn){
					case 1 : //未报名，但报名成功后马上可以开始学习
						contentTipBtnText = "label_core_training_management_278";//马上点这里开始学习
						contentFlag = true;
					break;
					case 4 : //未报名，但该课程是需要报名审批通过后才能学习的情况
						contentTipDesc = "label_core_training_management_276";//该课程需要先提交学习申请，等管理员或上司审批通过才能开始学习
						contentTipBtnText =　"label_core_training_management_278" //马上申请！
						contentFlag = true;
					break;
					case 7:  //未报名，但名额已满
					...
					case -1://未报名，但还没开始报名，或报名时间已过
						...
						//contentTipBtnText =　"btn_enrollment" 
				...
				if( ("C" == appStatus || "F" == appStatus) && (result.btn == 1 || result.btn == 4) ){//已完成（包括 ：已完成和未完成），但允许重复学习而且是允许报名的
                    ...
                    if(itm_retake_ind != 0){ //设置允许重读 
					   var btnHtml = "<span style='float: left;'>"+"<a onclick='"+contentTipBtnAction+"' class='btn wzb-btn-orange wzb-btn-big margin-right15' href='javascript: void(0);' style='margin-left: 10px;'>"+fetchLabel('label_core_training_management_282')+"</a></span>";
                       $('.bm.margin-top15').append($(btnHtml));  
                      }
					...
					//contentTipBtnText =　"label_core_training_management_281" //知道了！我只浏览一下
					//contentTipBtnAction = "showContent()";
					var btnHtml = "<span class='margin-right175' style='float:left'>"+Wzb.getButton(data, result.userEntId)+"</span>"+"</br>";
					$('.bm.margin-top15').append($(btnHtml)); 
					contentFlag = true;
				}else if(result.btn == 6){ //已报名，但还在等待队列
					var btnContent = "<span style='float:left'>"+Wzb.getButton(data, result.userEntId)+"</span>"+"</br>";
	                $('.bm.margin-top15').append(btnContent); 
					contentTipDesc = "label_core_training_management_284";//客官，別急，目前學習人數太多，請耐心等待（label已改为：名额已满）。
                    //contentTipBtnText =　"label_core_training_management_281" //知道了！我只浏览一下
					//contentTipBtnText =　"label_core_training_management_344" //
					...
					if(val.id < 1){
						...
						$.each(val.children, function(j,res){
							if(res.resources){
								...
								if(resObj.mov){
									$.extend(resObj.mov,{statusStr : getAppStatusStr(resObj.mov.mov_status,resObj.res_subtype,resObj.trainingStatus)});
								}
								hasChildFlag = true;
	...
	/* 					if($(".menuPane .dirMd .moduleDir").length >0 ){
			...
			if($(".xyd-wang .dirMd .moduleDir").length >0){
				if($(".xyd-wang .noDirMd .xyd-wang-bigbox").length > 0){
					//在没有文件夹的模块前面加上下划线
					$(".xyd-wang .noDirMd").prepend("<hr/>");//分割线
				}
...
/**********************网上课程****************************************************************************/
			...
			$(".xyd-wang-bigbox").bind('hover', function(){
			 ...
			 var html = $.trim(el.children(".xyd-wang-info").find(".moduletext:last").html())
						...
						+ $.trim(el.children(".xyd-wang-info").find(".div-warn:last").html());
			...
			$(".xyd-wang .xyd-pane-title").live('click', function(){
				...
				$(this).find("span i").removeClass("fa-plus-square").addClass("fa-minus-square");
				$(this).siblings().find("span i").addClass("fa-plus-square").removeClass("fa-minus-square");
			...
			if(child != undefined && child != ''){
				...
				$(".xyd-wang .xyd-pane-info:eq(0)").show();
				$(".xyd-wang .xyd-pane-title:eq(0)").addClass("skin-color");
				$(".xyd-wang .xyd-pane-title:eq(0)").find("i").addClass("fa-minus-square").removeClass("fa-plus-square");
			}
 				...
 				if(ccr.cmt_lst && ccr.cmt_lst.length>0) {
					...
					$.each(ccr.cmt_lst, function(i,val){
						...
						if(val.res != undefined && val.res !='' && val.res.res_type != undefined) {
							if(val.res.res_type == 'GLO' ) {
								status = fetchLabel('module_need_preview');//"需查閱";
							} else if(val.res.res_type == 'NETG_COK' || val.res.res_type == 'VOD' || val.res.res_type == 'RDG' || val.res.res_type == 'REF') {
								if(status == 'IFCP'){
									status = fetchLabel('status_viewed');//"需查閱"
								} else {
									status = fetchLabel('module_need_pass'); //"需完成";
								}
							} else if(val.res.res_type == 'SCO' || val.res.res_type == 'AICC_AU') {
								if(status == 'IFCP'){
									status = fetchLabel('module_need_attempt');//"需查閱"
								} else {
									status = fetchLabel('module_need_pass'); //"需完成";
								}
							} else if(val.res.res_type == 'ASS' || val.res.res_type == 'DXT' || val.res.res_type == 'TST'){
								if(status == 'IFCP' ){
									status = fetchLabel('module_need_submit');//需提交
								} else {
									status = fetchLabel('module_need_passed'); //"需合格"
								}
							} else if(val.res.res_type == 'SVY' || val.res.res_type == 'SCO'){
								if(status == 'CP' ){
									status = fetchLabel('module_need_submit');//需提交
								} else {
						...
						if(val.mov && val.cmt_status && val.cmt_status.indexOf(val.mov.mov_status) > -1){
							if(val.res && (val.res.res_type == 'TST' || val.res.res_type == 'ASS' || val.res.res_type == 'DXT')){
								if(val.cmt_lrn_pass_ind){
...
function showModule(obj, rcov, result){
	...
	if(obj.mov){
		$.extend(obj.mov,{statusStr : getAppStatusStr(obj.mov.mov_status)});
	}
	...
	//创建文字div
	$(baseObj).parent().next().find(".div-temp").empty().append($.render.moduleWarnField({})).append($.render.moduleField({text: obj.res_desc}));
...
//		if(app != undefined && app != '' && tkhId != undefined && tkhId > 0 ){
		//测验
		if(obj.res_subtype == 'DXT' || obj.res_subtype == 'TST'){
			...
			if(obj.res_duration == -1) {
				fields.push({text: fetchLabel('module_timelimit') + '：' + fetchLabel('module_unlimited')});	//限时		分钟
			} else {
				fields.push({text: fetchLabel('module_timelimit') + '：' + obj.res_duration + fetchLabel('time_minute')});
			}
...
//
			if(appOK && is_time_valide && is_inprogress && completed_pre_mod){
				if(obj.isRestore && (obj.mod.mod_max_usr_attempt == 0 || (obj.mod.mod_max_usr_attempt != 0 && obj.mov.mov_total_attempt < obj.mod.mod_max_usr_attempt))){
		...
		} else if(obj.res_subtype == 'ASS') {
			//作业
			if(appOK && is_time_valide && is_inprogress && completed_pre_mod) {
				// url = "javascript:module_lst.start_exec('"+ obj.res_subtype + "', '" + obj.res_id + "', 'blank_template.xsl', '" + result.resId + "', '${label_lan}', '" + tkhId + "', '', '', '${mod_lst.aicc_data.status}', '', " + isMobile + ", '" + userEntId + "','', '"+mod_eff_end_datetime+"' ,'"+itm_content_eff_end_time+"');";
				if(appOK && is_time_valide && is_inprogress && completed_pre_mod) {
					if(obj.pgr_attempt_nbr<=0  && (!obj.mod.ass_due_datetime || curDate <= obj.mod.ass_due_datetime)){
						url = "javascript:ass.submit_step_one('" + obj.res_id + "', '', '" + tkhId + "', " + isMobile + ", '" + userEntId + "');";
					}
				}
				//url = "javascript:module_lst.start_exec('"+ obj.res_subtype + "', '" + obj.res_id + "', 'blank_template.xsl', '" + result.resId + "', '${label_lan}', '" + tkhId + "', '', '', '${mod_lst.aicc_data.status}', '', " + isMobile + ", '" + userEntId + "','', '"+mod_eff_end_datetime+"' ,'"+itm_content_eff_end_time+"');";
			}
			if(appOK && (obj.mov != undefined && obj.mov != '') && obj.mov.mov_total_attempt > 0 && (obj.submit_num != undefined && obj.submit_num > 0)){
				url = "javascript:ass.view_result('" + obj.res_id + "', '" + userEntId + "', " + isMobile + ", '" + tkhId + "');";
			}
			//展开
			var	btnUrl,btnName;
			/* if(appOK && is_time_valide && is_inprogress && completed_pre_mod) {
				if(obj.pgr_attempt_nbr<=0  && (!obj.mod.ass_due_datetime || curDate <= obj.mod.ass_due_datetime)){
					btnUrl = "javascript:ass.submit_step_one('" + obj.res_id + "', '', '" + tkhId + "', " + isMobile + ", '" + userEntId + "');";
					btnName = fetchLabel('module_submit_job');	//提交作业
					$(baseObj).parent().next().find(".div-btn:last").append($.render.btnTemplate({
						name : btnName,
						href : btnUrl
			...
			if(appOK && (obj.mov != undefined && obj.mov != '') && obj.mov.mov_total_attempt > 0 && (obj.submit_num != undefined && obj.submit_num > 0)){
				btnUrl = "javascript:ass.view_result('" + obj.res_id + "', '" + userEntId + "', " + isMobile + ", '" + tkhId + "');";
				btnName = fetchLabel('module_attainment');	//成绩
			...
			//作业展开结束
		} else if(obj.res_subtype == 'SVY') {
			//调查问卷
			if(appOK && is_time_valide && is_inprogress && completed_pre_mod && (obj.mov == undefined || (obj.mov && obj.mov.mov_status != 'C' && obj.mov.mov_status != 'P')) ){
				url = "javascript:module_lst.start_exec('"+ obj.res_subtype +"', '" + obj.res_id + "', '" + getStyleSheet(obj.res_subtype) + "', '" + result.resId + "', '${label_lan}', '" + tkhId + "', '', '', '', '', " + isMobile + ", '" + userEntId + "','', '"+mod_eff_end_datetime+"','"+itm_content_eff_end_time+"');";
			}
		} else if(obj.res_subtype == 'REF'){
			if(obj.mod != undefined && obj.mod.mod_required_time != undefined && obj.mod.mod_required_time > 0){
				var desc = fetchLabel("lab_course_spend_reference_desc") + obj.mod.mod_required_time + fetchLabel("lab_course_spend_desc");
				if('${lang}'== 'en-us'){
					desc = fetchLabel("lab_course_spend_desc") + obj.mod.mod_required_time + fetchLabel("lab_course_spend_reference_desc");
				}
				$(baseObj).parent().next().find(".div-warn").html(desc);
			}
			//参考
			if(appOK && is_time_valide && is_inprogress && completed_pre_mod){
				url = "javascript:module_lst.start_ref_exec('"+ obj.res_subtype +"', '" + obj.res_id + "', '" + getStyleSheet(obj.res_subtype) + "', '" + result.resId + "', '" + tkhId + "', " + isMobile + ", '" + userEntId + "', '"+mod_eff_end_datetime+"','"+itm_content_eff_end_time+"');";
			} else if(appOK && is_time_valide && !is_inprogress){
				url = "javascript:module_lst.preview_ref('"+ obj.res_subtype +"', '" + obj.res_id + "', 'ist_reference_preview.xsl', '" + obj.res_id + "', " + isMobile + ");";
				//修改 按钮为【回顾】
				$(baseObj).siblings(".xyd-mwd05").find(".wzb-icon-begin").text(fetchLabel('module_review'));
			}

		} else if(obj.res_subtype == 'AICC_AU'){

			//AICC课件
			var fields = [];
			fields = [];
			if(obj.mod.mod_max_score > 0 ){
				fields.push({text: fetchLabel('module_full_score') + '：' + (obj.mod.mod_max_score > 0?obj.mod.mod_max_score : '--')});			//满分
				fields.push({text: fetchLabel('module_passing_score') + '：' + (obj.mod.mod_pass_score > 0? obj.mod.mod_pass_score : '--')});		//合格分数
			}
			if(obj.res_duration > 0 ){
				fields.push({text: fetchLabel('module_content_duration') + '：' + (obj.res_duration > 0 ? obj.res_duration+fetchLabel('time_minute') : '--')});	//限时		分钟
			}
			$(baseObj).parent().next().find(".div-temp").empty().append($.render.moduleField(fields));
			$(baseObj).parent().next().find(".div-temp").append($.render.moduleBtnField({text:''}));
			if(appOK && is_time_valide && is_inprogress && completed_pre_mod){
				url = "javascript:module_lst.start_aicc_au('" + obj.res_src_link + "', '" + userEntId + "', '"+result.resId+"', '"+obj.res_id+"', '" + obj.mod.mod_web_launch + "', '" + obj.mod.mod_vendor + "', '"+tkhId+"', false,'"+mod_eff_end_datetime+"','"+itm_content_eff_end_time+"');";
			} else if (appOK){
				if(appOK && is_time_valide && !is_inprogress ){
					url = "javascript:module_lst.preview_aicc_au('"+ obj.res_src_link + "', '', '', '', '" + obj.mod.mod_web_launch + "');";
					//修改 按钮为【回顾】
					$(baseObj).siblings(".xyd-mwd05").find(".wzb-icon-begin").text(fetchLabel('module_review'));
				}
			}
			if(appOK && obj.mov != null && obj.mov != '' && appOK){
				if(obj.mod.mod_vendor == 'SkillSoft'  && obj.mov.mov_total_attempt > 0)	//&& mod_lst.aicc_data.number > 0
				{
					//如果不是考试
					btnUrl = "javascript:rpt.lrn_skillsoft_rpt('"+ result.resId +"', '" + obj.res_id + "', '" + userEntId + "', '" + tkhId + "');";
					btnName = fetchLabel('module_study_report');	//学习报告
					$(baseObj).parent().next().find(".div-btn:last").append($.render.btnTemplate({
						name : btnName,
						href : btnUrl
					}));
				} else if(obj.mod.mod_vendor != 'SkillSoft' && obj.mov.mov_total_attempt > 0  && app)	//&& mod_lst.aicc_data.number > 0
				{
					//如果不是考试
					btnUrl = "javascript:rpt.aicc_rpt('"+ result.resId +"', '" + obj.res_id + "', '" + userEntId + "', isMobile, '" + tkhId + "',true);";
					btnName = fetchLabel('module_study_report');	//学习报告
					$(baseObj).parent().next().find(".div-btn:last").append($.render.btnTemplate({
						name : btnName,
						href : btnUrl
			...
			if(obj.res_duration > 0 ){
				fields.push({text: fetchLabel('module_content_duration') + '：' + (obj.res_duration > 0 ? obj.res_duration+fetchLabel('time_minute') : '--')});	//内容时长
			}
			$(baseObj).parent().next().find(".div-temp").empty().append($.render.moduleField(fields));
			$(baseObj).parent().next().find(".div-temp").append($.render.moduleBtnField({text:''}));
			//scorm课件
			if(appOK && is_time_valide && is_inprogress && completed_pre_mod){
				btnName = fetchLabel('module_detail');	//详细信息
				btnUrl = "javascript:show_sco_detail('" + result.resId + "', '${mod_lst.title_}');";
				url = "javascript:module_lst.start_scorm('" + obj.res_src_link + "', '" + userEntId + "', '" + result.resId + "', '" + obj.res_id + "', '" + obj.mod.mod_web_launch + "', '" + obj.mod.mod_vendor + "', '"+ app.app_tkh_id +"', false, '${is_multi_sco}', '"+ obj.res_sco_version +"', '"+mod_eff_end_datetime+"' ,'"+itm_content_eff_end_time+"');";
			} else if(appOK && is_time_valide && !is_inprogress){
				// scorm课件已完成后显示回顾按钮,回顾课件只是预览的作用,不会提交到数据库
				url = "javascript:module_lst.preview_scorm('" + obj.res_src_link + "', '" + userEntId + "', '" + result.resId + "', '" + obj.res_id + "','" + obj.mod.mod_vendor + "', '"+ obj.res_sco_version +"');";
				//修改 按钮为【回顾】
				$(baseObj).siblings(".xyd-mwd05").find(".wzb-icon-begin").text(fetchLabel('module_review'));
			}
			...
			if(appOK && is_time_valide && is_inprogress && completed_pre_mod) {
				url = "javascript:module_lst.start_exec('" + obj.res_subtype + "', '" + obj.res_id + "', '" + getStyleSheet(obj.res_subtype) + "', '" + result.resId + "', '${label_lan}', '" + tkhId + "', '"+(obj.mod.mod_max_usr_attempt)+"', '${mod_lst.attempt_nbr}', '${mod_lst.aicc_data.status}', '', " + isMobile + ", '" + userEntId + "','', '"+mod_eff_end_datetime+"' , '"+itm_content_eff_end_time+"');";
			} else if(appOK && is_time_valide && !is_inprogress){
				if(obj.res_subtype == 'RDG' || obj.res_subtype == 'GLO' || obj.res_subtype == 'VOD'){
					url = "javascript:module_lst.preview_exec('" + obj.res_subtype + "', '" + obj.res_id + "', '" + getStyleSheet(obj.res_subtype) + "', '" + obj.res_id + "', " + isMobile + ")";
					//修改 按钮为【回顾】
					$(baseObj).siblings(".xyd-mwd05").find(".wzb-icon-begin").text(fetchLabel('module_review'));
				}
			}
			if((obj.res_subtype == 'RDG' || obj.res_subtype == 'VOD') && obj.mod != undefined && obj.mod.mod_required_time != undefined && obj.mod.mod_required_time > 0){
				var warn = fetchLabel("lab_course_spend_vod_desc");
				if(obj.res_subtype == 'RDG'){
					warn = fetchLabel("lab_course_spend_learning_desc");
				}
				var desc = warn + obj.mod.mod_required_time + fetchLabel("lab_course_spend_desc");
				if('${lang}'== 'en-us'){
					desc = fetchLabel("lab_course_spend_desc") + obj.mod.mod_required_time + warn;
				}
				$(baseObj).parent().next().find(".div-warn").html(desc);
			}
		}
		...
		if(obj.mod && obj.mod.pre_res_title != undefined && obj.mod.pre_res_title!=''&& !obj.mod.pre_mod_had_completed && is_inprogress){
			...
			if(obj_mod_type == 'VOD' || obj_mod_type == 'RDG' || obj_mod_type == 'REF') {
				if(obj_mod_status == 'I'){
					la = fetchLabel('module_prep_requeid_viewed', [obj.mod.pre_res_title]); //需查阅
				} else {
					la = fetchLabel('module_prep_requeid_completed', [obj.mod.pre_res_title]); //需完成;
				}
			}else if(obj_mod_type == 'SCO' || obj_mod_type == 'AICC_AU' ) {
				if(obj_mod_status == 'I'){
					la = fetchLabel('module_prep_requeid_attempted', [obj.mod.pre_res_title]);//需尝试
				} else {
					la = fetchLabel('module_prep_requeid_completed', [obj.mod.pre_res_title]); //需完成
				}
...
//先修提示
function prompt_repair(pre_res_title){
	if(pre_res_title != undefined && pre_res_title != "")
		Dialog.alert(fetchLabel('module_prep_requeid', [pre_res_title]));
}
...
<div class="xyd-wang-bigbox" data-toggle="tooltip" data-placement="right" title="" data-original-title="">
	<div class="xyd-wang-title clearfix">
		<div class="xyd-mwd01">
			...
			<span style="display:none;" onclick="prompt_repair('{{>mod.pre_res_title}}')">{{>res_title}}</span>
		</div>
					...
					{{else}}
						{{if isTrainingCourse}}
							{{if trainingStatus.indexOf('[CP]') != -1}}
								...
								{{/if}}
							{{else}}
								{{if res_subtype == 'TST' || res_subtype == 'DXT'}}
									{{if isSubmit}}
										{{>mov.statusStr}} / <lb:get key="course_fulfill" /> <img src="${ctx}/static/images/Y.png" style="width:15px;height:15px;">
									{{else}}
										{{>mov.statusStr}} / <lb:get key="course_unfulfill" /> <img src="${ctx}/static/images/N.png" style="width:15px;height:15px;">
									{{/if}}
								{{else}}
									{{if (mov.mov_status.toLowerCase() == 'i' || mov.mov_status.toLowerCase() == 'c') && '[IFCP]' == trainingStatus}}
										{{>mov.statusStr}} / <lb:get key="course_fulfill" /> <img src="${ctx}/static/images/Y.png" style="width:15px;height:15px;">
									{{else}}
										{{>mov.statusStr}} / <lb:get key="course_unfulfill" /> <img src="${ctx}/static/images/N.png" style="width:15px;height:15px;">
									{{/if}}
...
<script id="conditionTemplate" type="text/x-jsrender">
	<p class="xyd-pane-title skin-color">
		<span>
			<i class="fa fa-plus-square"></i> {{>icd_type}}： {{>count}}
		</span>
	</p>

	<div class="xyd-pane-info menuInfo">
         <table class="table wzb-ui-table">
                <tbody>
					<tr class="wzb-ui-table-head">
                        <td width="50%"><!--课程名称 --><lb:get key='course_or_exam' /></td>
                        <td width="30%"><!--课程类型 --><lb:get key='course_or_exam_type' /></td>
...
<script id="courseTemplate" type="text/x-jsrender">
	<tr>
        <td>
			<a class="wzb-link02" href="${ctx}/app/course/detail/{{>itm_id}}{{if app}}?tkhId={{>app.app_tkh_id}}{{/if}}{{if app}}&{{/if}}{{if !app}}?{{/if}}isIntergrated=true&itgItemId=${itmId}&itgTkhId=${tkhId}" title=""><img src="${ctx}/static/images/pc-icon-p.png" width="26px" height="22px"/>{{if itm_mobile_ind == 'yes'}}<img src="${ctx}/static/images/phone-icon-g.png" width="21px" height="22px"/>{{/if}}{{:itm_title}}</a>
		</td>
        <td>{{>itm_type}}</td>
        <td align="right">
			...
			{{if cov}}
				<div style="width:60%;" class="progress progress-xs">
					<div class="progress-bar progress-bar-orange" style="width:{{if cov}}{{>cov.cov_progress == 0 || cov.cov_progress == undefined ? 10 : cov.cov_progress}}{{else}}0{{/if}}px">
					</div>
				</div>
				{{>cov.cov_status}}
    ...
    <dd>
		<img src="${ctx}{{:itm_icon}}" alt=""/>
         ...
         <div class="wzb-toolbox clearfix" id="courseSNS">
              <%--<a class="wzb-link03" href="javascript:;" title="" id="itm_like_count" style="text-align:center;"><i class="fa skin-color fa-thumbs-o-up" style="margin:0;"></i><p><!--赞--><lb:get key="sns_like"/>(<span>0</span>)</p></a>--%>
              <a class=" wzb-link03" href="javascript:;" title="" id="itm_collect_count" style="text-align:center;"><i class="fa skin-color fa-star" style="margin:0;"></i><p><!--收藏--><lb:get key="sns_collect"/>(<span >0</span>)</p></a>
         </div>
    </dd>
</script>
<!-- 按钮 -->
<script id="infoBtnTemplate" type="text/x-jsrender">
	...
	<div class="bm margin-top15">
        <!--报 名--><input type="button" onclick="javascript:void(0);" class="btn wzb-btn-orange wzb-btn-big margin-right15" value="<lb:get key="btn_enrollment"/>" name="frmSubmitBtn">
        <!--推荐给下属--><input type="button" onclick="javascript:void(0);" class="btn wzb-btn-orange wzb-btn-big" value="<lb:get key="btn_recommend_under"/>" name="frmSubmitBtn">
     </div>
	...
	<h3>
		...
		{{/if}} 
           {{if parent}}
             >
           {{/if}} 
		<c:if test="${isIntergrated}">
       		<a class="wzb-link01" href="${ctx}/app/course/detail/${itgItemId}?tkhId=${itgTkhId}" title="" >${itgItmTitle}</a> >
         </c:if> 
		    ...
		    <a class="wzb-link01" href="javascript:;" title="" >  </a>
	</h3>
...
<script id="infoTemplate" type="text/x-jsrender">
	...
	<dt>
		...
		<div class="offheight clearfix">
			<div class="offwidth">
				<span class="color-gray999"><!--课程类型--><lb:get key="course_type"/>：</span>{{>itemTypeStr}}
			</div>
			<div class="offwidth">
		...
		<p>
			<span class="color-gray999"><!--课程简介--><lb:get key="course_introduction"/>：</span>
...
<!--课程名称--><lb:get key="course_title"/>：
			  </td>
              <td class="wzb-table-content">{{:itm_title}}</td>
          </tr>
		  <tr>
              <td class="wzb-table-title" valign="top">
<!--  课程编号--><lb:get key="course_code"/>：</td>
              <td class="wzb-table-content">{{>itm_code}}</td>
          </tr>
          <tr>
              <td class="wzb-table-title" valign="top">
<!--课程类型--><lb:get key="course_type"/>：</td>
...
<!--培训中心--><lb:get key="traning_center"/>：
          ...
          <tr>
              <td class="wzb-table-title" valign="top">
<!--课程目录--><lb:get key="course_catalog"/>：
			  </td>
...
<!--证书--><lb:get key="certificate"/>：
		  ...
		  {{/if}}
          <tr>
              <td class="wzb-table-title" valign="top" style="padding-right:11px;">
<!-- 需要审批--><lb:get key="course_is_approval"/><div style="float:right;margin-right:-11px;">：</div></td>
...
<!--  报名期限--><lb:get key="enrollment_duration"/>：</td>
 ...
 <!-- 报名费--><lb:get key="course_fee"/>：</td>
		  ...
		  {{if ies && ies.ies_credit && !isOffline}}
          <tr>
              <td class="wzb-table-title" valign="top">
...
<!-- 学习对象--><lb:get key="course_audience"/>：</td>
          ...
          <tr>
              <td class="wzb-table-title" valign="top">
 <!-- 课程简介--><lb:get key="course_introduction"/>：</td>
              <td class="wzb-table-content">
				{{if itm_desc}}
					{{:itm_desc}}
				{{else}}
...
<script id="text-info-template" type="text/x-jsrender">
             <tr>
              <td class="wzb-form-label"  width="40%">{{>cg_alias}}：</td>
...
<tr>
    ...
    <td align="right"><span class="color-gray666">{{>msg_begin_date.substring(0,10)}}</span></td>
</tr>
...
<tr>
     <td><a class="wzb-link03" href="###" title="{{>cfc_title}}">{{>cfc_title}}</a></td>
     <td>{{>cfc_end_date}}</td>
...
<tr>
          ...
          <tr>
			  <!--班级地址 -->
              <td class="wzb-table-title" valign="top">
				<lb:get key="course_train_address"/> ：
			  </td>
              <td class="wzb-table-content">
				{{if ies}}
					{{if ies.ies_schedule}}
...
<script id="classInfoTemplate" type="text/x-jsrender">
    ...
    <dt>
		...
		<div class="offheight clearfix">
			<div class="offwidth">
				<span class="color-gray999"><!--培训日期--><lb:get key="course_class_period"/>：</span>
				<lb:get key="label_core_community_management_214"/> {{>itm_content_eff_start_time}} <lb:get key="time_to"/> {{>itm_content_eff_end_time}}
			</div>
		...
		<p>
			<span class="color-gray999"><!--班级简介--><lb:get key="class_introduction"/>：</span>
...
<script id="offflineInfoTemplate" type="text/x-jsrender">
    ...
    <dt>
         ...
         <p>
			<span class="color-gray999">
				<!--课程简介--><lb:get key="course_introduction"/>：
			</span>
			{{include tmpl="#openDescTemplate"/}}
			{{include tmpl="#infoBtnTemplate"/}}
    </dt>
...
<script id="classListTemplate" type="text/x-jsrender">
	<tr>
 	...
 	<tr>
        <td class="wzb-table-title"><span class="color-gray999"><!--班级编号--><lb:get key="class_code"/>：</span></td>
		<td class="wzb-table-content"> {{>itm_code}}</td>
		<td width="15%" align="right"><a class="btn wzb-btn-yellow" href="${ctx}/app/course/detail/{{>itm_id}}{{if app}}?tkhId={{>app.app_tkh_id}}{{/if}}" title="" ><!--详情--><lb:get key="btn_detail"/></a></td>
	</tr>
     <tr>
		  <td class="wzb-table-title"><span class="color-gray999"><!--培训地点--><lb:get key="course_train_address"/>：</span></td>
      ...
      <tr>
		  <td class="wzb-table-title" style="padding-bottom: 30px;"><span class="color-gray999"><!--培训期限--><lb:get key="course_train_interval"/>：</span></td>
...
<tr>
     ...
     <td>
		{{if res != null && res.res_type != null && res.res_type !=''  && (res.res_type =='TST' || res.res_type =='DXT')}} {{>cmt_pass_score*cmt_max_score/100}}
        {{else}}
			{{>cmt_pass_score}}
		{{/if}}
	 </td>
         ...
         <dl class="wzb-list-7 clearfix">
               <dd>
                  ...
                  <div class="wzb-user68">
                       <a style="cursor:default;" href="{{>a}}"><img class="wzb-pic" src="{{>image}}" alt="<lb:get key="know_ta"/>"></a>
               ...
               <dt>
					<!--去除class="wzb-link04"-->
                   <a  style = " font-size:14px; color:#00aeef; text-decoration:none; cursor:default;" href="{{>a}}" title="">{{>usr_display_bil}}</a>
                   <p>{{>usg_display_bil}}</p>
				...
				<dt>
					...
					<div class="bm margin-top15">
						<!-- <input type="button" onclick="javascript:void(0);"
							class="btn wzb-btn-blue wzb-btn-big margin-right15" value="报 名"
							name="frmSubmitBtn"> <input type="button"
							onclick="javascript:void(0);"
							class="btn wzb-btn-blue wzb-btn-big" value="推荐给下属"
							name="frmSubmitBtn"> -->
					</div>
				</dt>
			</dl>
			...
			<div role="tabpanel" class="wzb-tab-5" >
				...
				<div class="tab-content" id="box1" style="min-height:260px;">
					...
					<div role="tabpanel" class="tab-pane" id="dateList">
						<!--         日程表 -->
						<table class="table wzb-ui-table" style="display:none;">
							<tr class="wzb-ui-table-head">
					...
					<div role="tabpanel" class="tab-pane" id="notice">
						...
						<!-- myModal弹出框 Start-->
						<div id="msgBox" class="modal fade modal-gonggao" tabindex="-1" role="dialog"
						...
						<div>
							...
							<table class="wzb-table-two" id="cmt_lst">
								<tbody>
									...
									<tr>
                       					<td class="noline text-right" colspan="4"><span class="grayC999">(<i class="fa mL3 f18 fa-check"></i><!-- 为已完成此项要求 --><lb:get key="detail_complate_desc"/>)</span></td>
                    				</tr>
								</tbody>
							</table>
		...
		<c:if test="${hasInstrucotrs == true}">
			<div role="tabpanel" class="wzb-tab-5 margin-top15" id="evaluationDiv">
				<ul class="nav nav-tabs" role="tablist" id="nav2">
					<%-- <c:if test="${sns_enabled eq true}">
						<li role="presentation" class="active"><a href="#review"
							aria-controls="review" role="tab" data-toggle="tab"> <!-- 评 论_ -->
								<lb:get key="detail_valuation" />
						</a></li>
					</c:if> --%>
					<c:if test="${hasInstrucotrs == true}">
						<li role="presentation" class="active"><a href="#score" style="cursor:text;'" aria-controls="score"
								...
								<table>
									<tr><!--授课风格 -->
										...
										<td class="wzb-form-control" align="left">
											...
											<input type="hidden" name="itc_style_score" class="wzb-text-01" placeholder='<lb:get key="comment_max_score"/>' value="0">
										</td>
									</tr>
									<tr><!-- 教学质量 -->
										...
										<td class="wzb-form-control" align="left">
											...
											<input type="hidden" name="itc_quality_score" class="wzb-text-01" placeholder='<lb:get key="comment_max_score"/>'  value="0">
										</td>
									</tr>
									<tr><!--内容结构 -->
										...
										<td class="wzb-form-control" align="left">
											...
											<input type="hidden" class="wzb-text-01" name="itc_structure_score" placeholder='<lb:get key="comment_max_score"/>'  value="0">
										</td>
									</tr>
									<tr><!--交流互动 -->
										...
										<input type="hidden" class="wzb-text-01" name="itc_interaction_score"  placeholder='<lb:get key="comment_max_score"/>' value="0">
										</td>
									</tr>
									<tr id="lab_average"><!-- 平均分 -->
										...
										<td class="wzb-form-control" align="left">
											...
											<input type="hidden"  name="itc_score" class="wzb-text-01" readonly="readonly"  value="0">
										</td>
									</tr>
			...
			<script>
				$(function(){
				   ...
				   $("#submitInstructor").click(function(){
					   ...
					   //根据按钮判断是否报名
					  if(allResult.btn==1 || allResult.btn==4  || allResult.btn== 7 || allResult.btn==-1  || allResult.btn== 8){
			...
			<div id="TB_ajaxContent" style="width: 680px; height: 620px">
				<div class="wzb-model-3">
					<div class="form-search form-tool">
						<input name="searchContent" type="text" class="form-control" style="width:40%"
					...
					<div class="form-tool wzb-title-3">
						...
						<div class="form-tool-right">
							<input onclick="addAll()" type="button" id="addButton"
