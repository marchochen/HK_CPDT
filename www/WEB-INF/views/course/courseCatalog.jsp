...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#cpd_search").click(function() {
			if($("#max_cpd").val()!=''){
				if(!isNumber($("#max_cpd").val())){
					Dialog.alert(cwn.getLabel('label_core_cpt_d_management_102')+cwn.getLabel('label_core_cpt_d_management_105'));
					return;
				}
				
				if($("#max_cpd").val()<=0){ 
					Dialog.alert(cwn.getLabel('label_core_cpt_d_management_102')+cwn.getLabel('label_core_cpt_d_management_191'));
					return;
				}
			}

			
			if(!isNumber($("#min_cpd").val())){
				Dialog.alert(cwn.getLabel('label_core_cpt_d_management_103')+cwn.getLabel('label_core_cpt_d_management_105'));
				return;
			}
			
			if($("#max_cpd").val()!='' && ($("#max_cpd").val() < $("#min_cpd").val())){
				Dialog.alert(cwn.getLabel('label_core_cpt_d_management_106'));
				return;
			}
	...
	var colModel = [ {
		format : function(data) {
			...
			var schar_itm_desc= data.itm_desc ? data.itm_desc : "";
            var schar_str="";  
            var schar_len=0;  
            var schar;  
            var re = /[^\u4e00-\u9fa5]/;  
	...
	*/
	...
	var initHierarchyCatalog = function(aeTreeNodeVoList){
		...
		$(".wzb-list-31-li").hover(function(){
			//console.log($(this).find(".wzb-title-13-box span").length)
			if($(this).find(".wzb-title-13-box span").length<=3){
				$(this).find(".wzb-title-13").width('305')
			}
		})
	...
	$(document).ready(function() {
		$.getJSON("${ctx}/app/catalog/treeJson?cos_type=course", function(result) {
			//设置树结构
...
<dl class="wzb-list-6">
	 ...
	 <dt>
		  <p><a class="wzb-link01" href="${ctx}/app/course/detail/{{>encItmId}}"><img src="${ctx}/static/images/pc-icon-p.png" width="26px" height="22px"/>{{if itm_mobile_ind == 'yes'}}<img src="${ctx}/static/images/phone-icon-g.png" width="21px" height="22px"/>{{/if}}{{>title}}</a></p>
	                    ...
	                    <div class="wzb-title-13-title">
							{{for children}}
	                        	<div class="wzb-title-13-box">
									...
									{{if children && children.length > 0}}
										{{for children}}
	                            			<span class="wzb-title-13-biao"><a href="javascript:zTreeOnClick(null,null,{id:'{{>id}}'})">{{>name}}{{if children && children.length > 0}}{{/if}}</a></span>
										{{/for}}
									{{/if}}
	                        	</div>
							{{/for}}
    ...
    <div id="main" class="xyd-main clearfix">
        <div class="xyd-sidebar">
            ...
            <h3 class="wzb-title-4 skin-bg">
	            <lb:get key="label_core_training_management_297"/>
            ...
            <div class="wzb-model-8">
                 ...
                 <dl class="wzb-list-4" id="course_type">
                     <dt><lb:get key="course_type"/>：</dt>         
                     <dd><a href="javascript:;" title="" data=""><i class="fa margin-right10 fa-circle skin-color" ></i><lb:get key="status_all"/></a></dd>
                     <dd><a href="javascript:;" title="" data="selfstudy"><i class="fa margin-right10 fa-circle-thin" ></i><lb:get key="selfstudy"/></a></dd>
                     <dd><a href="javascript:;" title="" data="classroom"><i class="fa margin-right10 fa-circle-thin" ></i><lb:get key="classroom"/></a></dd>
                     <%-- <dd><a href="javascript:;" title="" data="integrated"><i class="fa margin-right10 fa-circle-thin"></i><lb:get key="integrated"/> </a></dd> --%>
                 </dl> 
                 ...
                 <c:if test="${cpd_search_ind==true}">
                 <dl class="wzb-list-4">
                    <dt><lb:get key="label_cpd.label_core_cpt_d_management_101" />：</dt>
                    <dd>
                    <input class="wzb-text-01" name=""  id="min_cpd" type="text" placeholder="<lb:get key="label_cpd.label_core_cpt_d_management_103" />">
                     -
                    <input class="wzb-text-01" name="" id="max_cpd" type="text" placeholder="<lb:get key="label_cpd.label_core_cpt_d_management_102" />">
                    <input style="margin:0 0 2px 10px;" name="" id="cpd_search" value="<lb:get key="label_cpd.label_core_cpt_d_management_104" />" type="button" class="btn wzb-btn-blue">
                    </dd>
                </dl>
                </c:if>
            </div>
        ...
        $(function(){
            $(".xyd-train-desc").click(function(){
                    ...
                    $.getJSON("${ctx}/app/catalog/treeJson?cos_type=course&tcr_id=" + tcrId, function(result) {
                    	//重新加载目录树，并清除目录的选择条件
                    	tndId = 0;
                        $.fn.zTree.init($("#zTree"), setting, result.aeTreeNodeVoList);
                        initHierarchyCatalog(result.aeTreeNodeVoList);
