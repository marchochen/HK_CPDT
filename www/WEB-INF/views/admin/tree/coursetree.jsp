...
<link rel="stylesheet" href="${ctx}/static/js/jquery-table/css/jquery-table-admin.css" />
  ...
  $(function(){
	  ...
	  var entIds = [];
	  //遍历课程标题
	  parent.$(".wzb-choose-detail").each(function(){
	       	 entIds.push(parent.$(this).text().trim());
	       	})
			var cids=parent.$("#qid").val();
	  if(cids!=""){
 	...
 	<div class="wzb-area-content" style="overflow-y: auto;height: 490px;">
		...
		<div role="tabpanel" class="wzb-tab-2" >
                        <ul class="nav nav-tabs" role="tablist">
                            ...
                            <span><lb:get key="label_tm.label_core_training_management_26"/> </span>
                            </ul>
...
var ars=items.split(",");
	  for (var i = 0; i < arr.length; i++) {
		  text+='<div id="send_user_'+arr[i]+'" class="wzb-choose-info" value="'+arr[i]+'">'+
	    	'<span class="wzb-choose-detail">'+ars[i]+'</span>' +'<a class="wzb-choose-area" href="javascript:void(0)" onclick="removeDiv('+arr[i]+')">  <i class="fa fa-remove"></i> </a></div>';
	  }
