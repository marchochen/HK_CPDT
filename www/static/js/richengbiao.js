var richeng = (function () {
  ...
  function createSelectYear() {
    ...
    for (var i = 0; i <= 79; i++) {
      yearSelect.options.add(new Option((i + 1970) + richeng.label.year, i + 1970));
      if (currYear == i + 1970) {
  ...
  function createMonthSelect() {
      ...
      for (var i = 0; i < 12; i++) {
        selectmonth.options.add(new Option(richeng.label['month' + (i + 1)], i + 1));
        if (currMonth == i) {
      ...
      withClass('aboluo-calendar-month').appendChild(selectmonth)
    }
    //根据传入的年月，创建对应的table日期,并且在每个td中创建a标签用于事件，与样式内边框的设置

  function createTabledate(year, yue) {
	  withClass("aboluo-month-a-perv").style.borderRight = '10px solid #00aeef'
	      withClass("aboluo-month-a-next").style.borderLeft = '10px solid #00aeef'
  ...
  function nextA(year, yue, day) {
	 if(year == Number(withID("aboluo-yearSelect").options[withID("aboluo-yearSelect").options.length-1].value) + 1){
		 return;
	 }else{
      ...
      for (var i = 0; i < rows.length; i++) {
        for (var j = 0; j < rows[i].cells.length; j++) {
          ...
          if (arr[0] == year && arr[1] == month + 1 && arr[2] == day) {
            ...
            if (AjaxDate) {
              new AjaxDate(false, false, a.getAttribute('date'),richeng.label)
            }
          }
        }
      }
  ...
  function setA() {
      ...
      for (var i = 0; i < arr.length; i++) {
        ...
        if (arr[i].parentNode.className == "aboluo-pervMonthDays") {
          arr[i].setAttribute("onclick", "javascript:richeng.pervA(" + datearr[0] + "," + datearr[1] + "," + datearr[2] + ",this);javascript:richeng.stopBubble(this);")
        } else if (arr[i].parentNode.className == "aboluo-nextMonthDays") {
          arr[i].setAttribute("onclick", "javascript:richeng.nextA(" + datearr[0] + "," + datearr[1] + "," + datearr[2] + ",this);javascript:richeng.stopBubble(this);")
        } else {
        	
          arr[i].setAttribute("onclick", "javascript:richeng.setRigth(" + datearr[0] + "," + datearr[1] + "," + datearr[2] + ");javascript:richeng.stopBubble(this);");
        }
  ...
  function setaclass(year, yue, day) {
      ...
      if (year1 == year && yue == month1 + 1 && day1 == day) {
    	 withClass('aboluo-currToday').className = 'aboluo-currToday aboluo-aclick'
      } else {
        var tbody = withClass("aboluo-rilitbody");
        var arr = tbody.getElementsByTagName("a");
        for (var i = 0; i < arr.length; i++) {
          ...
          if (datearr[0] == year && datearr[1] == yue && datearr[2] == day) {
        	  withClass('aboluo-currToday aboluo-aclick').className = 'aboluo-currToday';
        	  arr[i].setAttribute("class", "aboluo-aclick");
            if (AjaxDate) {
              if (arr[i].children.length > 0) {
                new AjaxDate(true,false, arr[i].getAttribute('date'),richeng.label)
              } else {
                new AjaxDate(false, false, arr[i].getAttribute('date'),richeng.label)
              }
            }
          }
        }
  ...
  function leftrightclick() {
      ...
      lefta.title = richeng.label.left;
      righta.title = richeng.label.right;
      righta.onclick = function () {
        ...
        if (monthvalue == 12) {
        	if(yearvalue == yearselect.options[yearselect.options.length-1].value){
        		righta.style.borderLeft = '10px solid #ddd'
        		return;
        	}else{
        		 yearvalue += 1;
      ...
      lefta.onclick = function () {
        ...
        if (monthvalue == 1) {
        	if(yearvalue == 1970){
        		lefta.style.borderRight = '10px solid #ddd'
        		return;
        	}else{
        		yearvalue -= 1;
                //这里已经变了一年了,所以就要根据年重读数据
  ...
  function DateLoading(getCall, lang) {
    var i18n = {
      "en-us": {
        "year": '',
        "month1": 'Janurary',
