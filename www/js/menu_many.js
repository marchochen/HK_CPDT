...
/*			if(frm.mod_type.value == 'TST'|| frm.mod_type.value == 'DXT'){
				if(frm.show_answer_ind.value == '1'){
					window.parent.location.href = wb_utils_invoke_servlet('cmd','get_rpt_usr_test_html','rpt_usr_id',frm.pgr_usr_id.value,'mod_id',frm.pgr_mod_id.value,'que_id_lst',0,'attempt_nbr','0','stylesheet','lrn_test_report.xsl')
				} else {
					window.parent.location.href = wb_utils_invoke_servlet('cmd','get_rpt_usr_test_html','rpt_usr_id',frm.pgr_usr_id.value,'mod_id',frm.pgr_mod_id.value,'que_id_lst',0,'attempt_nbr','0','stylesheet','lrn_test_report_count_only.xsl')
				}
				return;
			}
				...
				for (var j = 1; j <= m; j++){								
					...
					if( que.interactions[0].getType() == 'FB' && (frm.mod_type.value == 'SVY' || frm.mod_type.value == 'EVN') ) {
						if( int_ans.length > 1000 ) {
							alert(wb_msg_question  + i + wb_msg_enter_less_char);
							//if(forceSubmit == true){
		...
		if ( frm.mod_type.value != 'SVY' && frm.mod_type.value != 'EVN' ){			
			...
			if(frm.is_cos_eval && frm.is_cos_eval.value == 'true' && frm.mod_type.value != 'EVN'){
                if (diff_domain != 'true') {
    				...
    				frm.url_success.value = '../htm/close_and_reload_window.htm?no_reload=true';
                }
			} else if(frm.mod_type.value == 'EVN') {
				frm.url_success.value = '../htm/close_and_reload_window.htm?no_reload=false';
			}else{
				frm.url_success.value =  wb_utils_invoke_servlet('cmd','get_rpt_usr','rpt_usr_id',frm.pgr_usr_id.value,'mod_id',frm.pgr_mod_id.value,'que_id_lst',0,'attempt_nbr','0','stylesheet','lrn_tst_rpt_usr_header.xsl','tkh_id',frm.tkh_id.value)	
			}			
		}
...
function CwQuestionAutoSaveAll(save_by_usr) {
	var que_cnt = qset.getSize();
	submitCount = que_cnt;
	for (i = 1; i <= que_cnt; i++){
		CwQuestionAutoSave(i, save_by_usr);
	}
}
...
var QueRedFlagged = '../wb_image/que_red_flag.jpg';
var QueNomalFlagged = '../wb_image/que_normal_flag.jpg';
var QueIndSet = 'yes';
var QueIndUnset = '';
var SlotSize = 10;
...
function redrawSlotInd() {
	...
	for (var i = 1; i <= SlotSize; i++){
		...
		if(objtemp!=null){
			if (objtemp.value == '') {
				if (eval(FrmPrefix + 'flag' + convertSlot2Que(i)).value == '') {
					document.getElementById('slot_atm' + i).src = '';
					document.getElementById('slot_atm' + i).title = wb_msg_unanswer;           
				} else {
					document.getElementById('slot_atm' + i).src = QueIndFlagged;
					document.getElementById('slot_atm' + i).title = wb_msg_unanswer +'+'+ wb_msg_marked;
				}
			} else {
				if (eval(FrmPrefix + 'flag' + convertSlot2Que(i)).value == '') {
					document.getElementById('slot_atm' + i).src = QueIndAttempted;
					document.getElementById('slot_atm' + i).title = wb_msg_answered;
				} else {
					document.getElementById('slot_atm' + i).src = QueIndAttemptedAndFlagged;
					document.getElementById('slot_atm' + i).title = wb_msg_answered +'+'+ wb_msg_marked;
				}
			}
		}else{
			document.getElementById('slot_atm' + i).src = QueIndUnflag;
			document.getElementById('slot_atm' + i).title = '';
		}
	}
	*/
...
//	window.location.hash = slot_name;
    changeQue(slot_id);
    
//    var scrollheight = document.documentElement.scrollTop;
	var scrollheight = $("#layer1").height() - 35;
	for(var i=2;i<slot_id;i++){
		scrollheight += $("#layer" + i).height() + 35;
	}
	if(slot_id == 1){
		scrollheight = 0;
	}
...
function changeQue(que_id) {
	...
	/*
	recalPageOffset(que_id);
	var slot_prev_page_td =  document.getElementById('slot_prev_page_td');
	var slot_next_page_td =  document.getElementById('slot_next_page_td');
	if(curPageOffset==1){
		document.getElementById('slot_prev_page').style.display = 'none';
		slot_prev_page_td.onmouseover = '';
		slot_prev_page_td.onmouseout = '';
	}else{
		document.getElementById('slot_prev_page').style.display = '';
		slot_prev_page_td.onmouseover = function() {
			setFocusColor(slot_prev_page_td);
		};
		slot_prev_page_td.onmouseout = function() {
			unsetFocusColor(slot_prev_page_td);
		};
	}
	if(curPageOffset==total_page){
		document.getElementById('slot_next_page').style.display = 'none';
		slot_next_page_td.onmouseover = '';
		slot_next_page_td.onmouseout = '';
	}else{
		document.getElementById('slot_next_page').style.display = '';
		slot_next_page_td.onmouseover = function() {
			setFocusColor(slot_next_page_td);
		};
		slot_next_page_td.onmouseout = function() {
			unsetFocusColor(slot_next_page_td);
		};
	}
	unsetFocusColor(slot_prev_page_td);
	unsetFocusColor(slot_next_page_td);
...
function leaveQue(que_id) {
	CwQuestionGoto(que_id);
	curIdxObj = eval(FrmPrefix + 'atm' + que_id);
	if (curIdxObj.value == '') {
		if (qset.getQuestion(que_id).interactions[0].getType() == 'MT') {
			if(qset.getQuestion(que_id).answer_array.length >=1){
				attemptQue(que_id);
			}else{
				unattemptQue(que_id);
			}
		}else{
			if (qset.getQuestionStatus(que_id) == STS_COMPLETE) {
				attemptQue(que_id);
			} else {
				unattemptQue(que_id);
			}
		}
	}else{   //仅用于删除回答内容 题目标记为未回答  
		if (qset.getQuestion(que_id).interactions[0].getType() == 'MT') {
			if(qset.getQuestion(que_id).answer_array.length >=1){
				attemptQue(que_id);
			}else{
		...
		/*if (qset.getQuestionStatus(que_id) == STS_COMPLETE) {
			...
			console.log( qset.getQuestionStatus(que_id) + '---------' + STS_COMPLETE);
			console.log('a26666a  curIdxObj.value：'+curIdxObj.value + '---que_id:' + que_id);
			//unattemptQue(que_id);
		}*/
	}
	redrawSlotInd();
...
//    }
    if(eval(FrmPrefix + 'atm' + cur_que_id).value == ''){
    	document.getElementById('slot_td_idx' + cur_que_id).className = 'graycolor border-bottom-red';
    }else{
...
function attemptQue(que_id) {
	...
	//document.getElementById('slot_idx' + que_id).style.color = 'black';
...
function unattemptQue(que_id){
    ...
    if(eval(FrmPrefix + 'flag' + que_id).value != ''){
    	document.getElementById('slot_td_idx' + que_id).className = 'graycolor border-bottom-red';
    } else {
		document.getElementById('slot_td_idx' + que_id).className = 'graycolor';
    }
		//document.getElementById('slot_idx' + que_id).style.color = 'black';
...
//	document.getElementById('titlediv_hidden').style.height = document.getElementById('titlediv').offsetHeight;
	for (i = 1; i <=n; i++) {
...
//	if(document.getElementById('time_remain_value')){
    document.getElementById('time_remain_value').innerHTML = formatTimeRemain(timeLeft);
//	}
	
    if(document.getElementById('total_que_value')){
	...
	for(var i=1;i<=frmObj.total_que.value;i++){
		preSlotID = convertQue2Slot(i);
		changeQue(i);
    ...
    timeRemainIntervalObj = window.setInterval('refreshTimeRemain()', 1000);
    autoSavaInterval = Math.random()*2+3;

  if(submit_test){
  	autoSaveIntervalObj = window.setInterval('CwQuestionAutoSaveAll(true)', 30000);
 //      autoSaveIntervalObj = window.setInterval('CwQuestionAutoSaveAll(false)', autoSavaInterval*1000*60);
  }
}
...
function changeDragedPosition(event,posId) {
	...
	if(dragedObj.style.visibility == 'visible') {
		...
		dragedObj.style.left = getLeft(eventObj, posId) + X + DragedDivCursorOffset;
		dragedObj.style.top = getTop(eventObj, posId) + Y + DragedDivCursorOffset;
	}
}
...
//		srcEle.style.height   = OptionTextHeight * 5;
	}
}
...
//		srcEle.style.height   = OptionTextHeight;
	}
}
...
$(function(){
	if($(".fixedly").offset() !== undefined){
		var v_modTop = $(".fixedly").offset().top;
		var on_scroll = function(){
			...
			if(top > v_modTop){
				if('undefined' == typeof(document.body.style.maxHeight)){   //兼容iE6
					$('.fixedly').addClass('abs');
					$('.fixedly').css('top',$(window).scrollTop());
				}else{
					$('.fixedly').addClass('fixed ie');
				}
			}else{
				$('.fixedly').removeClass('abs');
				$('.fixedly').removeClass('fixed ie');
			}
		}
