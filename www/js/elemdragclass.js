...
function MM_dragInit() {
  ...
  with (this) {
    ...
    if (_obj) {
       _width =  parseInt(_obj.offsetWidth);
       _height = parseInt(_obj.offsetHeight);
      getPosition();
...
function MM_dragUpdate(dragging) {
  ...
  with (this) {
    if (dragging) {
      if (this.onDrag != null) {
        x = parseInt((_parent.browserIsNS)?_obj.style.left:_obj.style.pixelLeft);
        y = parseInt((_parent.browserIsNS)?_obj.style.top:_obj.style.pixelTop);
...
function MM_dragGetPosition() {
  if (this._obj) with (this) {
    ...
    _posX = parseInt((_parent.browserIsNS)?_obj.style.left:_obj.style.pixelLeft);
    _posY = parseInt((_parent.browserIsNS)?_obj.style.top:_obj.style.pixelTop);
...
function MM_dragSnapTo(newPosX, newPosY) {
  if (this._obj && newPosX && newPosY) with (this) {
    if (_parent.browserIsNS) {
      _obj.style.left = newPosX; _obj.style.top = newPosY;
    } else {
