...
jQuery.extend(Boxy, {
    ...
    DEFAULTS: {
        ...
        closeText:              null,      // text to use for default close link
...
Boxy.prototype = {
    ...
    _setupTitleBar: function() {
        if (this.options.title) {
            ...
            if (this.options.closeable) {
                tb.append(jQuery("<a href='#' hidefocus='true' class='close'></a>").html(this.options.closeText));
            }
...
jQuery.extend(Boxy, {
	iframe : function(url, options) {
		...
		var iframeContent = '<div class=\'boxy-iframe-content\'><iframe border="0" scrolling="no" frameborder="0" src="' + url + '">You need a Browser which can display iframes</iframe></div>';
		var box = new Boxy(iframeContent, options);

		jQuery('.boxy-iframe-content iframe').load(function() {
			var width = $('.boxy-iframe-content iframe').contents().find('#container').width() + 10;
			var height = $('.boxy-iframe-content iframe').contents().find('#container').height() + 10;

			var width = $('.boxy-iframe-content iframe').width(width);
			var height = $('.boxy-iframe-content iframe').height(height);
			box.resize(width, height, function() {
			});
		});
	}
});
