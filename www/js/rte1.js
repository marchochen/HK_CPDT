...
function getHTML()
{
	...
	while(szRet.indexOf("<A href=")>-1){
		szRet = szRet.replace("<A href=","<A target=\"_blank\" href=");
	}
	return szRet
}
...
function insertHTML(szHTML)
{
	//if(g_state.isSelected()) {
		...
		if (g_state.bMode) {
			...
			idEditbox.focus();
			//win.selection.createRange().pasteHTML(szHTML);
			idEditbox.document.selection.createRange().pasteHTML(szHTML);
...
{
	frmXml.content.value = idEditbox.document.body.innerHTML
	return
}

function setContent(_content)
{
	idEditbox.document.body.innerHTML = _content
	return
}
...
//alert("2" + objBody.innerHTML)
		}
		_setSize()
		frmXml.cbMode.checked = !bMode
		_Editor_MoveSelection(true)
		setFocus()
	}
	return bMode
...
function _setSize() {
	//document.all.idEditbox.style.pixelHeight = document.body.clientHeight - idToolbar.offsetHeight - document.all.idMode.offsetHeight - 20
	document.all.idEditbox.style.pixelHeight = 190;
...
function _pageReady()
{
	...
	/*
	if (g_state.szSearch!=""){
		document.domain = g_state.szSearch
		idEditbox.document.domain = document.domain
		idPopup.document.domain = idEditbox.document.domain
	}
	*/
...
{
	var aIds = new Array("cut","copy","paste","bar1","formatblock","fontstyle","fontsize","bar2","bold","italic","underline","bar3","left","center","right","bar4","orderedlist","unorderedlist","outdent","indent","bar5","line","link","fb","table","image","bar6","bgcolor","textcolor","bar7","emoticon","file")
	var aTips = new Array(L_TIPCUT_TEXT,L_TIPCOPY_TEXT,L_TIPPASTE_TEXT,"",L_TIPP_TEXT,L_TIPFSTYLE_TEXT,L_TIPFSIZE_TEXT,"",L_TIPB_TEXT,L_TIPI_TEXT,L_TIPU_TEXT,"",L_TIPLJ_TEXT,L_TIPCJ_TEXT,L_TIPRJ_TEXT,"",L_TIPOL_TEXT,L_TIPUL_TEXT,L_TIPDINDENT_TEXT,L_TIPIINDENT_TEXT,"",L_TIPLINE_TEXT,L_TIPLINK_TEXT,L_TIPFB_TEXT,L_TIPTABLE_TEXT,L_TIPIMAGE_TEXT,"",L_TIPBGCOLOR_TEXT,L_TIPFGCOLOR_TEXT,"",L_TIPEMOTICON_TEXT,L_TIPFILE_TEXT)
	var aCommand = new Array("_Format('cut')","_Format('copy')","_Format('paste')",null,"_CPopup_Show('formatblock')","_CPopup_Show('font')","_CPopup_Show('fontsize')",null,"_Format('bold')","_Format('italic')","_Format('underline')",null,"_Format('Justify','Left')","_Format('Justify','Center')","_Format('Justify','Right')",null,"_Format('insertorderedlist')","_Format('insertunorderedlist')","_Format('outdent')","_Format('indent')",null,"_Format('InsertHorizontalRule')","_CPopup_Show('Link')","_CPopup_Show('Fb')","_CPopup_Show('Table')","_CPopup_Show('Image')",null,"_CPopup_Show('BackColor')","_CPopup_Show('ForeColor')",null,"_CPopup_Show('Emoticon')","_CPopup_Show('File')")
	...
	var sz = "<DIV ID=idStandardBar style='width:100%;'><NOBR>", iLeft=0, iHeight=24
	for (var i = 0 ; i < aSizes.length; i++)
	{
		if(aIds[i]!="image" && aIds[i]!="bgcolor" && aIds[i]!="link" && aIds[i]!="fb" && aIds[i]!="table" && aIds[i]!="emoticon"){
		sz	+=  ""
...
function _drawModeSelect(lang) {
	if (frmXml.cbMode) {frmXml.cbMode.checked = false;}
}
...
function _CState()
{
	...
	this.isSelected = _CSatate_IsSelected
}
...
function _CPopup_Show(szType)
{
	...
	else
	{
		...
		if ((!oRenderer) || ("Link"==szType) || ("Fb"==szType))
...
function _CPopupRenderer_Display(szType) {
	...
	if (oRenderer.autoSize) {
		...
		idPopup.document.all.puRegion.style.pixelHeight  = document.all.idPopup.style.pixelHeight = 150
		idPopup.document.all.puRegion.style.pixelWidth  = document.all.idPopup.style.pixelWidth = document.body.clientWidth - 600
		document.all.idPopup.style.pixelLeft = 25
	}
	if(szType == 'Image' || szType == 'File') {
		idPopup.document.all.puRegion.style.pixelHeight  = document.all.idPopup.style.pixelHeight = 150
		idPopup.document.all.puRegion.style.pixelWidth = document.all.idPopup.style.pixelWidth = 250
	}
...
function _CPopupRenderer(szType)
{
	...
	switch(szType)
	{
		...
		case "fontsize":
			...
			break
		case "Link":
			this.szCaption		= wb_msg_ins_upd_link
			this.PrepareHTML	= _CLinkPopupRenderer_PrepareHTML
			this.szHTML			= this.PrepareHTML()
			break
		case "Fb":
			this.szCaption		= L_PUTITLEFB_TEXT
			this.PrepareHTML	= _CFbPopupRenderer_PrepareHTML
			this.szHTML			= this.PrepareHTML()
			break
		case "Table":
			...
			break
		case "Image":
			this.szCaption		= wb_msg_upload_img
			this.PrepareHTML	= _CImgFilePopupRenderer_PrepareHTML
			this.szHTML			= this.PrepareHTML()
			break
		case "File":
			this.szCaption		= wb_msg_upload_file
			this.PrepareHTML	= _CImgFilePopupRenderer_PrepareHTML
			this.szHTML			= this.PrepareHTML()
			break
		case "BackColor":
...
function _CImgFilePopupRenderer_PrepareHTML()
{
	return "<iframe name='upload_file_iframe' style='height:76%;width:92%;' frameBorder='no' marginWidth=0 scrolling='yes' align='left'></iframe>";
}
...
function _CFbPopupRenderer_AddFb(d) 
{
	...
	if (szURL!="") 
	{
		...
		if (d.all.urlValue.value!="")
			oSel.execCommand("CreateFb",false,szURL)	
	}

	idEditbox.focus()	
}

function _CFbPopupRenderer__UpdateURL(oDoc,szURL) {
...
function _CFbPopupRenderer_PrepareHTML() 
{
	...
	if (g_state.aLinks.length>0) 
	{
		...
		+	   "<TD>" 
		+		   L_FBSELECT_TEXT 
		+		   "<SELECT ID=pageList ONCHANGE=\"parent._CFbPopupRenderer__UpdateURL(this.document,this[this.selectedIndex].value)\">"
		+			   "<OPTION VALUE=''>" 
		+				   "=="
		+				   L_FBSELECTPAGE_TEXT
		+				   "=="
	...
	+	   "<TR>"
	+		   "<TD ALIGN=center>"
	+			   "<INPUT ONCLICK=\"parent._CFbPopupRenderer_AddFb(this.document)\" TYPE=submit ID=idSave VALUE=\"" + L_INSERT_TEXT + "\"> <INPUT ONCLICK=\"parent._CPopup_Hide()\" TYPE=reset ID=idCancel VALUE=\"" + L_CANCEL_TEXT + "\">"
	+		   "</TD>"
	+	   "</TR>"
	+   "</TABLE>"
	return sz
...
function _CLinkPopupRenderer_PrepareHTML()
{
	...
	sz  += ""
	+   "<BR>"
	+   "<NOBR><SELECT ID=urlType STYLE=\"background: #FCEEDE;color: #000000;font: 8pt Verdana, Geneva, Arial, Helvetica, sans-serif;\">"
	...
	sz += "</SELECT><INPUT STYLE=\"background: #FCEEDE;border: 1px solid;border-color: #C99F6F;font: 8pt Verdana, Geneva, Arial, Helvetica, sans-serif;scrollbar-base-color: #DEB98E;\" ID=urlValue SIZE=45 VALUE=\"" + szURL + "\" TYPE=text></NOBR>"
	...
	+	   "<TR>"
	+		   "<TD ALIGN=center>"
	+			   "<INPUT STYLE=\"background-color: #FCEEDE;border: 1px outset #A1774A;color: #634D35;cursor: hand;font: 9pt Verdana, Geneva, Arial, Helvetica, sans-serif;\" ONCLICK=\"parent._CLinkPopupRenderer_AddLink(this.document)\" TYPE=\"button\" ID=idSave VALUE=\"" + wb_msg_ok + "\"> <INPUT STYLE=\"background-color: #FCEEDE;border: 1px outset #A1774A;color: #634D35;cursor: hand;font: 9pt Verdana, Geneva, Arial, Helvetica, sans-serif;\" ONCLICK=\"parent._CPopup_Hide()\" TYPE=reset ID=idCancel VALUE=\"" + wb_msg_cancel + "\">"
	+		   "</TD>"
	+	   "</TR>"
	+   "</TABLE>"
	return sz
...
function _CUtil_getSelImageMML()
{
  var oSel, oEl, szMml
  ...
  if (oSel.type!="None") {
    oEl = _CUtil_GetElement(oSel.item(0),"IMG")
      szMml = oEl.getAttribute("mml", 2)
  }
  return szMml
}


function _CFormula_Insert()
{
   ...
   winStats+='scrollbars=no,width=625,height=300'
   ...
   sel_mml = _CUtil_getSelImageMML()
   if ((sel_mml!=null) && (sel_mml!=""))
     wbUtilsOpenWin("../jsp/math/PopUp.jsp?mml="+sel_mml,"",false,winStats)
   else
     wbUtilsOpenWin("../jsp/math/PopUp.jsp","",false,winStats)
   return
}

function setImgContent(filePath){
	var img_html =  "<IMG src=\"" + filePath + "\" style=\"border:0\" " + "onload=\"javascript:if(this.width>600)this.width=600\" isupdated>";
        insertHTML(img_html);
}

function setFileContent(fileName, filePath){
	var file_html =  "<IMG align=\"bottom\" src=\"../wb_image/icon_unknow.gif\"><A href=\"" + filePath + "\" isupdated>" + fileName + "</A>"
        insertHTML(file_html);
}

function setMediaDisplay(szType) {
	if(szType == "Image" || szType == "File") {
		var upload_file_iframe = document.frames("idPopup").document.frames("upload_file_iframe").document;

		var sz = "<html>"
			+   "<STYLE>"
			+	   ".Text {font: 9pt Verdana,Geneva,Arial,Helvetica,sans-serif}"
			+	   ".InputFrm {background: #FCEEDE;border: 1px solid;border-color: #C99F6F;font: 8pt Verdana, Geneva, Arial, Helvetica, sans-serif;scrollbar-base-color: #DEB98E;}"
			+	   ".Btn {background-color: #FCEEDE;border: 1px outset #A1774A;color: #634D35;cursor: hand;font: 9pt Verdana, Geneva, Arial, Helvetica, sans-serif;}"
			+   "</STYLE>"
			+ " <body><form id='upload_file_frm' method='post' enctype='multipart/form-data'>"
			+ "<input type='hidden' name='module'>"
			+ "<input type='hidden' name='cmd'>"
			+ "<input type='hidden' name='src_filename'>";

		if(szType == "Image") {
			sz += "<input type='hidden' name='upload_type' value='img'>";
		} else {
			sz += "<input type='hidden' name='upload_type' value='file'>";
		}

		sz += "<input type='hidden' name='rename'>"
			+ "<input type='hidden' name='stylesheet'>"
			+ "<table border=0><tr><td class=\"Text\">";

		if(szType == "Image") {
			sz += wb_msg_select_uploaded_picture;
		} else {
			sz += wb_msg_select_uploaded_file;
		}

		sz += ':</td></tr>'
			+ "<tr><td><input class=\"InputFrm\" name='upload_file' type='file'/></td></tr>"
			+ "<tr><td><input class=\"Btn\" type='button' onClick=\"parent.parent.uploadFile(this.form)\" value=\"" + wb_msg_upload + "\"/> "
			+ "<input  class=\"Btn\" onclick=\"parent.parent._CPopup_Hide()\" type=reset value=\"" + wb_msg_cancel + "\">"
			+ "</td></tr></table></form></body></html>";
		upload_file_iframe.open("text/html","replace");
		upload_file_iframe.write(sz);
		upload_file_iframe.close();
	}
}

function uploadFile(frm){
	var file_path = frm.upload_file.value;
	var upload_type = frm.upload_type.value;
	if(upload_type == 'img') {
		if(file_path == ''){
	...
	if(upload_type == 'file') {
		if(file_path == ''){
			alert(wb_msg_select_uploaded_file);
			return false;
		}
		var fileExt = getFileExt(file_path);
		fileExt = fileExt.toLowerCase();
		if(fileExt=='app'||fileExt=='bat'||fileExt=='cgi'||fileExt=='com'||fileExt=='exe'||fileExt=='pif'||fileExt=='vb'||fileExt=='ws'||fileExt=='sh'||fileExt=='msi'||fileExt=='js'){
			alert(wb_msg_file_type_limit);
			return false;
		}
	}
	...
	frm.src_filename.value = getFileName(file_path);
	frm.cmd.value = 'announ_upload_file'
	frm.module.value = 'upload.UploadModule'
	//frm.rename.value = 'yes';
	frm.action = '../../../servlet/Dispatcher';
	frm.stylesheet.value = 'announ_upload_msg_box.xsl';
	frm.submit();
}
