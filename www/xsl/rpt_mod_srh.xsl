...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
					...
					<tr>
						<td>
							<xsl:call-template name="wb_goldenman_sel_mod">
								...
								<!--
												<xsl:with-param name="on_set_value_function">document.getElementById('sso_link_4').checked = true;enableRemoveBtn('mod_id');</xsl:with-param>
												-->
								...
								<xsl:with-param name="dis_mod_type">1</xsl:with-param>
								<!--1: 所有 ；2：只第一层 ; 3: 只测验类  -->
						...
						<xsl:for-each select="/report/report_body/display_option/other/object_view/attribute">
							...
							<xsl:if test="@sortable='yes'">
								<option value="{.}">
									...
									<xsl:choose>
										<xsl:when test="$current='att_status'">
					...
					<table cellpadding="0" cellspacing="0" width="100%" border="0">
						...
						<tr>
							...
							<td>
								<label for="display">
									<span class="Text">
										<xsl:value-of select="$lab_show_course_only"/>
									</span>
								</label>
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<label for="undisplay">
									<span class="Text">
										<xsl:value-of select="$lab_show_course_all"/>
										<xsl:text>：</xsl:text>
									</span>
								</label>
							</td>
						...
						<tr>
							...
							-->
							<td>
								<xsl:apply-templates select="*[name() != 'item']" mode="content">
									<xsl:with-param name="lab_item_content" select="$lab_item_content"/>
									<xsl:with-param name="lab_run_content" select="$lab_run_content"/>
									<xsl:with-param name="lab_others_content" select="$lab_others_content"/>
									<xsl:with-param name="lab_run_num" select="$lab_run_num"/>
	...
	<xsl:template match="other" mode="content">
		...
		<table cellpadding="0" cellspacing="0" width="100%" border="0">
			<tr>
				<td colspan="3">
					<span class="Text"><xsl:value-of select="$lab_others_content"/>：</span>
				</td>
			</tr>
			<tr>
