...
<xsl:variable name="img_path"><xsl:value-of select="$wb_img_path"/></xsl:variable>
	...
	<xsl:variable name="str_action">
		<xsl:choose>
			<xsl:when test="@itemtype='FDR'">javascript:folder_read_mode('<xsl:value-of select="@title"/>');</xsl:when>
			<xsl:otherwise>javascript:ugr_read_mode('<xsl:value-of select="@identifierref"/>');</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	
	<xsl:variable name="is_folder">
		<xsl:choose>
			<xsl:when test="@itemtype='FDR'">YES</xsl:when>
	...
	<xsl:variable name="has_child">
		...
		<xsl:choose>
			<xsl:when test="$node_image != $img_path">
				<tree srcid="{@identifierref}" text="{@title}" title="{@title}" identifier="{@identifier}" itemtype="{@itemtype}" restype="{@restype}" is_folder="{$is_folder}" has_child="{$has_child}" icon="{$node_image}" >
					<xsl:apply-templates select="item" mode="node"/>
				</tree>			
			</xsl:when>			
			<xsl:otherwise>
				<tree srcid="{@identifierref}" text="{@title}" title="{@title}" identifier="{@identifier}" itemtype="{@itemtype}" restype="{@restype}" is_folder="{$is_folder}" has_child="{$has_child}" action="{$str_action}">
					<xsl:apply-templates select="item" mode="node"/>
				</tree>			
			</xsl:otherwise>
		</xsl:choose>
