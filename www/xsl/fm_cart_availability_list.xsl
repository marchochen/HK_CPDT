...
<!-- ====================================================================== -->	
	...
	<xsl:template name="lang_ch">
		<xsl:apply-templates>
			...
			<xsl:with-param name="lab_g_form_btn_reserve">預約</xsl:with-param>
			<xsl:with-param name="lab_new_reservation">新增預訂</xsl:with-param>
			<xsl:with-param name="lab_check_availability">查看使用情況</xsl:with-param>
			...
			<xsl:with-param name="lab_no_record">沒有預定記錄</xsl:with-param>
			<xsl:with-param name="lab_reserved_by_other">設施正被其他新增的預定</xsl:with-param>
		</xsl:apply-templates>
	</xsl:template>
	<xsl:template name="lang_gb">
		<xsl:apply-templates>
			<xsl:with-param name="lab_g_form_btn_cancel">取消</xsl:with-param>
			<xsl:with-param name="lab_g_form_btn_reserve">预约</xsl:with-param>
			<xsl:with-param name="lab_new_reservation">新增预订</xsl:with-param>
			<xsl:with-param name="lab_check_availability">显示可用设施</xsl:with-param>
			...
			<xsl:with-param name="lab_no_record">没有可用设施</xsl:with-param>
			<xsl:with-param name="lab_reserved_by_other">设施正被其他用户预定</xsl:with-param>
		</xsl:apply-templates>
	</xsl:template>
	<xsl:template name="lang_en">
		<xsl:apply-templates>
			<xsl:with-param name="lab_g_form_btn_cancel">Cancel</xsl:with-param>
			<xsl:with-param name="lab_g_form_btn_reserve">Reserve</xsl:with-param>
			<xsl:with-param name="lab_new_reservation">New reservation</xsl:with-param>
			<xsl:with-param name="lab_check_availability">Check availability</xsl:with-param>
			...
			<xsl:with-param name="lab_no_record">No facilities are available</xsl:with-param>
	...
	<xsl:template match="fm">
		...
		<xsl:param name="lab_check_availability"/>
		...
		<xsl:param name="lab_g_form_btn_reserve"/>
		
		<xsl:call-template name="fm_head"/>
		<xsl:call-template name="wb_ui_title">
			<xsl:with-param name="text"><xsl:value-of select="$lab_new_reservation"/> - <xsl:value-of select="$lab_check_availability"/></xsl:with-param>
		</xsl:call-template>
		...
		<xsl:choose>
			<xsl:when test="count(facility_availability/facility_schedule_list/facility_schedule) != 0">		
				<xsl:apply-templates select="facility_availability" mode="fac_avail">
					...
					<xsl:with-param name="default_checked">false</xsl:with-param>						
				</xsl:apply-templates>
			</xsl:when>
			<xsl:otherwise>
		...
		<div class="wzb-bar">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:call-template name="wb_gen_form_button">
						<xsl:with-param name="wb_gen_btn_name" select="$lab_g_form_btn_reserve"/>
