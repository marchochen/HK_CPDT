...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template name="lang_ch">
		<xsl:call-template name="content">
			<xsl:with-param name="lab_reference">修改參考</xsl:with-param>
	...
	<xsl:template name="lang_gb">
		<xsl:call-template name="content">
			<xsl:with-param name="lab_reference">修改参考</xsl:with-param>
	...
	<xsl:template name="lang_en">
		<xsl:call-template name="content">
			<xsl:with-param name="lab_reference">Edit reference</xsl:with-param>
		...
		<form name="frmXml" onSubmit="return status()">
			...
			<table>
				<tr>
					...
					<td class="wzb-form-control">
						<input type="text" name="ref_title" size="15" style="width:300px;" value="{reference/@ref_title}" class="wzb-inputText"/>
					</td>
				</tr>
				<tr>
					<td class="wzb-form-label">
					...
					<td class="wzb-form-control">
						<input type="text" name="ref_url" size="15" style="width:300px;" value="{reference/@ref_url}" class="wzb-inputText"/>
					</td>
				</tr>
				<tr>
					...
					<td class="wzb-form-control">
						<textarea rows="6" wrap="VIRTUAL" style="width:300px;" cols="35" name="ref_description" class="wzb-inputTextArea">
							<xsl:value-of select="reference/description/text()"/>
						</textarea>
					</td>
				</tr>
			</table>
			<div class="wzb-bar">
				<xsl:call-template name="wb_gen_form_button">
					...
					<xsl:with-param name="wb_gen_btn_href">javascript:ref.upd_ref(document.frmXml,'<xsl:value-of select="$wb_lang"/>')</xsl:with-param>
				</xsl:call-template>
				<xsl:call-template name="wb_gen_form_button">
			...
			<input type="hidden" name="ref_id" value="{reference/@ref_id}"/>
		</form>
	</xsl:template>
	<!-- =============================================================== -->
