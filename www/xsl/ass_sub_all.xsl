...
queue = 'all'	
	...
	<xsl:template name="lang_ch">
		<xsl:call-template name="content">
			...
			<xsl:with-param name="lab_not_submitted">尚未提交</xsl:with-param>
	...
	<xsl:template name="lang_en">
		<xsl:call-template name="content">
			...
			<xsl:with-param name="lab_not_submitted">Not submitted</xsl:with-param>
	...
	<xsl:template name="content">
		...
		<xsl:param name="lab_not_submitted"/>
		...
		<xsl:call-template name="wb_ui_desc">
			<!-- <xsl:with-param name="text"><xsl:value-of select="$lab_view_submission"/></xsl:with-param> -->
		</xsl:call-template>
		<xsl:call-template name="wb_ui_hdr">
			<xsl:with-param name="belong_module">FTN_AMD_ITM_COS_MAIN</xsl:with-param>
			<xsl:with-param name="parent_code">HOMEWORK_CORRECTION</xsl:with-param>
		</xsl:call-template>
		<table cellpadding="0" cellspacing="0" style="margin-top:0px;" border="0" width="{$wb_gen_table_width}">
			<tr>
				<td width="660">
		...
		<xsl:choose>
			<xsl:when test="count(record) > 0">
				<table class="table wzb-ui-table" id="wzb-ui-table-pager">
					...
					<xsl:call-template name="do_rol">
						<xsl:with-param name="lab_not_submitted" select="$lab_not_submitted"/>
	...
	<xsl:template name="do_rol">
		...
		<xsl:for-each select="record">
			...
			<tr class="{$row_class}">
				<td>
					<xsl:choose>
						<xsl:when test="progress/@status = 'Not Submitted Yet' ">
							<span class="Text">
								<xsl:value-of select="entity"/> (<xsl:value-of select="@usr_id"/>)</span>
						</xsl:when>
						<xsl:otherwise>
							<a href="javascript:ass.grade({$mod_id},'{entity/@ent_id}','{progress/@tkh_id}','',{$isFromIframe},{$cos_id})" class="Text">
