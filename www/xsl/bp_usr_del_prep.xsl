...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:java="http://xml.apache.org/xalan/java" exclude-result-prefixes="java">
	...
	<xsl:template name="lang_ch">
		<xsl:call-template name="content">
			...
			<xsl:with-param name="lab_upload_instruction2">注意﹕系統會在背景處理上載的請求。請點擊“查看記錄”按鈕以查看完成結果。</xsl:with-param>
	...
	<xsl:template name="lang_gb">
		<xsl:call-template name="content">
			...
			<xsl:with-param name="lab_upload_instruction2">注意：系统会在后台处理上载的请求。 请点击“查看记录”按钮以查看完成结果。</xsl:with-param>
	...
	<xsl:template name="lang_en">
		<xsl:call-template name="content">
			...
			<xsl:with-param name="lab_desc"><i style="color:#64BF0F;">Reminder：</i>Specify the file containing the user info below. The file must be in a specific format according to a spreadsheet template.</xsl:with-param>
			...
			<xsl:with-param name="lab_upload_instruction2">Note: The import process will be conducted in a separate background job .Please click the “View history” button to check the completion result.</xsl:with-param>
	...
	<xsl:template name="content">
		...
		<xsl:call-template name="wb_ui_hdr">
			...
			<xsl:with-param name="page_title">
				<xsl:call-template name="get_lab">
					<xsl:with-param name="lab_title">label_um.label_core_user_management_58</xsl:with-param>
				</xsl:call-template>
			</xsl:with-param>
		</xsl:call-template>
		...
		<tr>	
				<td class="wzb-form-label">
					<xsl:call-template name="wb_gen_form_button">
						...
						<xsl:with-param name="wb_gen_btn_href">javascript:Batch.User.Import.get_del_tpl('<xsl:value-of select="$cur_lan"/>', '<xsl:value-of select="$cur_site"/>')</xsl:with-param>
					...
					<xsl:call-template name="wb_gen_form_button">
						...
						<xsl:with-param name="wb_gen_btn_href">javascript:Batch.User.Import.get_del_instr()</xsl:with-param>
						<xsl:with-param name="class">btn wzb-btn-orange margin-right4</xsl:with-param>
					</xsl:call-template>
					<xsl:call-template name="wb_gen_form_button">
						<xsl:with-param name="wb_gen_btn_name" select="$lab_g_txt_btn_log"/>
						<xsl:with-param name="wb_gen_btn_href">javascript:Batch.User.Log.get_del_log()</xsl:with-param>
		...
		<div class="wzb-bar">
			<xsl:call-template name="wb_gen_form_button">
				...
				<xsl:with-param name="wb_gen_btn_href">javascript:Batch.User.Import.del_exec(document.frmXml,'<xsl:value-of select="$wb_lang"/>')</xsl:with-param>
			</xsl:call-template>
			<xsl:call-template name="wb_gen_form_button">
