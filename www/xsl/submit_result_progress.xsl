...
function send_test() {
	window.parent.document.getElementById('all_frameid').rows = wb_utils_testplayer_fs_row_progress;
	timer = window.setInterval("send_test_progress()",1000);
}

function send_test_progress(){
	var send_test_pro_url = wb_utils_invoke_disp_servlet('module', 'report.ReportProgressModule', 'cmd', 'get_rpt_pro', 'window_name', window_name)
	req = getXMLHttpRequest();
	req.onreadystatechange = onChange;
	req.open("GET", send_test_pro_url,true);
	req.send(null);
}
...
function downloadComplete() {
	...
	sent_stop_flag();
			...
			<body leftmargin="0" marginwidth="0" marginheight="0" topmargin="0" onload="send_test()">
	...
	<xsl:template name="lang_ch">
		<xsl:call-template name="content">
			<!-- header text -->
			<xsl:with-param name="lab_up_desc">正在提交測驗，請稍候...</xsl:with-param>
			<xsl:with-param name="lab_no_record">提交試題數為零</xsl:with-param>
			<xsl:with-param name="lab_error">提交測驗過程中發生錯誤，請聯絡系統管理員，錯誤描述如下:</xsl:with-param>
			<xsl:with-param name="lab_complete">測驗提交成功。</xsl:with-param>
		</xsl:call-template>
	</xsl:template>
	<xsl:template name="lang_gb">
		<xsl:call-template name="content">
			<!-- header text -->
			<xsl:with-param name="lab_up_desc">正在提交测验，请稍候...</xsl:with-param>
			<xsl:with-param name="lab_no_record">提交试题数为零</xsl:with-param>
			<xsl:with-param name="lab_error">提交测验过程中发生错误，请联系系统管理员，错误描述如下:</xsl:with-param>
			<xsl:with-param name="lab_complete">测验提交成功。</xsl:with-param>
		</xsl:call-template>
	</xsl:template>
	<xsl:template name="lang_en">
		<xsl:call-template name="content">
			<xsl:with-param name="lab_up_desc">Your answers are being submitted...</xsl:with-param>
			<xsl:with-param name="lab_no_record">No record found</xsl:with-param>
			<xsl:with-param name="lab_error">Error occurred during submitting test(please contact system administrator):</xsl:with-param>
			<xsl:with-param name="lab_complete">Test submission completed.</xsl:with-param>
		</xsl:call-template>
	</xsl:template>
	<!-- =============================================================== -->
	<xsl:template name="content">
		<xsl:param name="lab_submit_result"/>
