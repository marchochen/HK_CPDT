...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:import href="share/gen_rpt_mod_prep.xsl"/>
	...
	<xsl:template name="lang_ch">
		<xsl:call-template name="pre_content">
			...
			<xsl:with-param name="lab_no_restriction">使用於所有時期</xsl:with-param>
			<xsl:with-param name="lab_start_date">開始日期</xsl:with-param>
			<xsl:with-param name="lab_end_date">結束日期</xsl:with-param>
			<xsl:with-param name="lab_usage_between">使用於</xsl:with-param>
	...
	<xsl:template name="lang_gb">
		<xsl:call-template name="pre_content">
			...
			<xsl:with-param name="lab_criteria_desc">为创建课程使用报告指定条件。</xsl:with-param>
			...
			<xsl:with-param name="lab_no_restriction">所有时间内的使用情况</xsl:with-param>
			<xsl:with-param name="lab_start_date">起始日期</xsl:with-param>
			<xsl:with-param name="lab_end_date">截止日期</xsl:with-param>
			<xsl:with-param name="lab_usage_between">以下时间内的使用情况</xsl:with-param>
	...
	<xsl:template name="lang_en">
		<xsl:call-template name="pre_content">
			...
			<xsl:with-param name="lab_criteria_desc">Specify the criteria for generating usage report.</xsl:with-param>
			...
			<xsl:with-param name="lab_no_restriction">Usage in all period</xsl:with-param>
			<xsl:with-param name="lab_start_date">Start date</xsl:with-param>
			<xsl:with-param name="lab_end_date">End date</xsl:with-param>
			<xsl:with-param name="lab_usage_between">Usage between</xsl:with-param>
	...
	<xsl:template name="pre_content">
		...
		<xsl:call-template name="content">
			...
			<xsl:with-param name="lab_usage_between" select="$lab_usage_between"/>
			<xsl:with-param name="lab_view_report" select="$lab_view_report"/>
			<xsl:with-param name="btn_view_rpt_link">rpt.usage_mod_lst_in_date</xsl:with-param>
			<xsl:with-param name="parent_code" select="$parent_code"/>
