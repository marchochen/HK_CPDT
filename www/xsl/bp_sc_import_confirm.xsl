...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template name="lang_ch">
		<xsl:call-template name="content">
			...
			<xsl:with-param name="lab_desc">下列是檔中的情景題。按螢幕底部的<b>確定</b> 按鈕完成匯入，或按<b>取消</b> 按鈕停止操作。</xsl:with-param>
			...
			<xsl:with-param name="lab_err_invalid_xls_sheet">在上傳文件中未找到名稱為“Data”的工作表。包含導入資料的工作表名稱必須為“Data”。</xsl:with-param>
			<xsl:with-param name="lab_err_invalid_spec_score">無效的"抽題條件"</xsl:with-param>
			...
			<xsl:with-param name="lab_err_invalid_shuffle_lines">無效的"可更改選項次序"選項</xsl:with-param>
			<xsl:with-param name="lab_err_invalid_sc_shuffle_lines">無效的"情景題子題目可更改次序"選項</xsl:with-param>
	...
	<xsl:template name="lang_gb">
		<xsl:call-template name="content">
			...
			<xsl:with-param name="lab_desc">下列是文件中的情景题。单击屏幕底部的<b>确定</b> 按钮完成导入，或单击<b>取消</b> 按钮停止操作。</xsl:with-param>
			...
			<xsl:with-param name="lab_err_invalid_xls_sheet">在上传文件中未找到名称为“Data”的工作表。包含导入数据的工作表名称必须为“Data”。</xsl:with-param>
			<xsl:with-param name="lab_err_invalid_spec_score">无效的"抽题条件"</xsl:with-param>
			...
			<xsl:with-param name="lab_err_invalid_shuffle_lines">无效的"可更改选项次序"选项</xsl:with-param>
			<xsl:with-param name="lab_err_invalid_sc_shuffle_lines">无效的"情景题子题目可更改次序"选项</xsl:with-param>
	...
	<xsl:template name="lang_en">
		<xsl:call-template name="content">
			...
			<xsl:with-param name="lab_err_invalid_xls_sheet">The worksheet named “Data” cannot be found in the uploaded file. The worksheet that is containing the data to be imported must be named “Data”.</xsl:with-param>
			<xsl:with-param name="lab_err_invalid_spec_score">Invalid criteria</xsl:with-param>
			...
			<xsl:with-param name="lab_err_invalid_shuffle_lines">Invalid shuffle option</xsl:with-param>
			<xsl:with-param name="lab_err_invalid_sc_shuffle_lines">Invalid shuffle</xsl:with-param>
			...
			<xsl:with-param name="lab_err_invalid_desc_lines">Invalid eescription</xsl:with-param>
	...
	<xsl:template name="content">
		...
		<xsl:param name="lab_err_invalid_spec_score"/>
		...
		<xsl:param name="lab_err_invalid_sc_shuffle_lines"/>
				...
				<table width="{$wb_gen_table_width}" border="0" cellspacing="0" cellpadding="3" class="Bg">
					<tr>
						...
						<td>
							...
							<table cellpadding="3" cellspacing="0" border="0" width="100%">
								<xsl:for-each select="*[line/@num != '0']">
									<tr>
										<td width="10">
											<img src="{$wb_img_path}tp.gif" width="1" height="1" border="0"/>
										</td>
										<td colspan="3">
											<span class="Text">
												...
												<xsl:choose>
													...
													<xsl:when test="name() = 'invalid_spec_lines'">
														<xsl:value-of select="$lab_err_invalid_spec_score"/>
													</xsl:when>
													<xsl:when test="name() = 'invalid_shuffle_lines'">
														<xsl:value-of select="$lab_err_invalid_shuffle_lines"/>
													</xsl:when>
													<xsl:when test="name() = 'invalid_sc_shuffle_lines'">
														<xsl:value-of select="$lab_err_invalid_sc_shuffle_lines"/>
													</xsl:when>
													<xsl:when test="name() = 'invalid_criteria_lines'">
														<xsl:value-of select="$lab_err_invalid_spec_score"/>
													</xsl:when>
				...
				<xsl:choose>
					<xsl:when test="count(question) &gt;=1">
						<table border="0" cellspacing="0" cellpadding="0" width="{$wb_gen_table_width}" class="wzb-ui-table">
							...
							<xsl:apply-templates select="question[@type='FSC' or @type='DSC']">
								...
								<xsl:with-param name="lab_spec_empty" select="$lab_spec_empty"/>
								<xsl:with-param name="que_type" select="question/@type"/>
							</xsl:apply-templates>
						</table>
					</xsl:when>
					<xsl:otherwise>
	...
	<xsl:template match="question">
		...
		<tr valign="middle" class="{$row_class}">
			...
			<xsl:apply-templates select="../../used_column/column" mode="draw_item">
				...
				<xsl:with-param name="lab_no_explanation" select="$lab_no_explanation"/>
				<xsl:with-param name="lab_spec_empty" select="$lab_spec_empty"/>
				<xsl:with-param name="que_type" select="$que_type"/>
			</xsl:apply-templates>
			<td width="4">
				<img src="{$wb_img_path}tp.gif" width="1" height="1" border="0"/>
			</td>
		...
		<xsl:choose>
			...
			<xsl:when test="@id = '7'">
				<td nowrap="nowrap" valign="top" align="center">
					<span class="wbRowText">
					<xsl:choose>
						<xsl:when test="$que_type = 'DSC'">
							<xsl:choose>
								<xsl:when test="$question/header/dsc_criteria/text() != ''">
									<xsl:value-of select="$question/header/dsc_criteria/text()"/>
								</xsl:when>
								<xsl:otherwise>
									<font color="#FF0000"><xsl:value-of select="$lab_spec_empty"/></font>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:when>
						<xsl:otherwise>
