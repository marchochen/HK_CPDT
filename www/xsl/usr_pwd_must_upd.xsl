...
<xsl:variable name="entity_assignment_cnt"><xsl:value-of select="count(/user_manager/user/all_role_list/role/entity_assignment[contains(@type,'USG')])"/></xsl:variable>
...
<xsl:template name="lang_ch">
	<xsl:call-template name="content">
		<xsl:with-param name="lab_change_pwd">更改<xsl:value-of select="$lab_passwd"/></xsl:with-param>	
		<xsl:with-param name="lab_pwd_info"><xsl:value-of select="$lab_passwd"/>資料</xsl:with-param>
		<xsl:with-param name="lab_pwd_desc">你的<xsl:value-of select="$lab_passwd"/>已無效，現在請更改<xsl:value-of select="$lab_passwd"/>。</xsl:with-param>
...
<xsl:template name="lang_gb">
	<xsl:call-template name="content">
		...
		<xsl:with-param name="lab_pwd_desc">你的<xsl:value-of select="$lab_passwd"/>已无效，现在请更改<xsl:value-of select="$lab_passwd"/>。</xsl:with-param>
...
<xsl:template name="lang_en">
	<xsl:call-template name="content">
		<xsl:with-param name="lab_change_pwd">Change <xsl:value-of select="$lab_passwd"/></xsl:with-param>	
		<xsl:with-param name="lab_pwd_info"><xsl:value-of select="$lab_passwd"/> information</xsl:with-param>
		<xsl:with-param name="lab_pwd_desc">Your <b><xsl:value-of select="$lab_passwd"/></b> is no longer effective. Please change your <xsl:value-of select="$lab_passwd"/> now.</xsl:with-param>
	...
	<body leftmargin="0" marginwidth="0" marginheight="0" topmargin="0" onload="wb_utils_gen_form_focus(document.frmXml)">
		<form name="frmXml">			
			...
			<xsl:call-template name="wb_ui_hdr">
				<xsl:with-param name="show_home">fale</xsl:with-param>
				<xsl:with-param name="show_back">fale</xsl:with-param>
				<xsl:with-param name="show_functions">fale</xsl:with-param>
			</xsl:call-template>
			<xsl:call-template name="wb_ui_title">
				<xsl:with-param name="text" select="$lab_change_pwd"/>
			</xsl:call-template>
		
			<xsl:call-template name="wb_ui_desc">
				<xsl:with-param name="text" select="$lab_pwd_desc"/>
			</xsl:call-template>
...
<xsl:template name="draw_detail_lst">
	...
	<xsl:call-template name="usr_detail_seperate_hdr">
		<xsl:with-param name="hdr_title"><xsl:value-of select="$lab_pwd_info"/></xsl:with-param>
