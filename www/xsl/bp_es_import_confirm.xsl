...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:java="http://xml.apache.org/xalan/java" exclude-result-prefixes="java">
	...
	<xsl:variable name="lab_title_error" select="java:com.cwn.wizbank.utils.LabelContent.get($wb_cur_lang, 'label_tm.label_core_training_management_189')"/>
	<xsl:variable name="lab_title_too_long_error" select="java:com.cwn.wizbank.utils.LabelContent.get($wb_cur_lang, 'label_tm.label_core_training_management_380')"/>
										...
										<td colspan="3">
											<span class="wbRowText">
												...
												<xsl:choose>
													...
													<xsl:when test="name() = 'invalid_title_too_long_lines'">
														<xsl:text>"</xsl:text><xsl:value-of select="$lab_title_error"/><xsl:text>"&#160;</xsl:text><xsl:value-of select="$lab_title_too_long_error"/>
													</xsl:when>
							...
							<xsl:otherwise>
								<xsl:if test="$upload_success_cnt &gt;= 1">
									<xsl:call-template name="wb_gen_form_button">
										...
										<xsl:with-param name="wb_gen_btn_href">javascript:Batch.Res.Import.confirm(document.frmXml,'ES')</xsl:with-param>
									</xsl:call-template>
								...
								<xsl:call-template name="wb_gen_form_button">
									...
									<xsl:with-param name="wb_gen_btn_href">
										<xsl:choose>
											...
											<xsl:otherwise>javascript:Batch.Res.Import.cancel(<xsl:value-of select="$ulg_id"/>, 'ES')</xsl:otherwise>
										</xsl:choose>
									</xsl:with-param>
								</xsl:call-template>
							</xsl:otherwise>
		...
		<xsl:choose>
			...
			<xsl:when test="@id = '3'">
				<td nowrap="nowrap" valign="top" align="left">
					<span class="wbRowText">
<!--
						<xsl:value-of select="$question/body/text()"/>
...
-->
			...
			<xsl:when test="@id = '4'">
				<td nowrap="nowrap" valign="top" align="left">
					<span class="wbRowText">
						<xsl:call-template name="unescape_html_linefeed">
							<xsl:with-param name="my_right_value"><xsl:value-of select="$question/explanation/rationale"/></xsl:with-param>
						</xsl:call-template>
					</span>
				</td>
			</xsl:when>						
			...
			<xsl:when test="text() = 'Answer'">
				...
				<td nowrap="nowrap" valign="top" align="left">
					<span class="wbRowText">
						...
						<!--
						<xsl:value-of select="$question/body/interaction/option[@id = $outcome_condition]/."/>
						-->
					</span>
				</td>
			</xsl:when>
			...
			<xsl:when test="@id = '9'">
				<td nowrap="nowrap" valign="top" align="left">
					<span class="wbRowText">
						<xsl:choose>
							<xsl:when test="$question/header/@que_submit_file = 'true'">
								<xsl:value-of select="$lab_yes"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="$lab_no"/>
