...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:variable name="wb_gen_table_width">510</xsl:variable>
	<!-- =============================================================== -->
	<xsl:template match="/">
		...
		<html>
			<head>
				...
				page_timestamp = ']]><xsl:call-template name="escape_js">
						<xsl:with-param name="input_str" select="$page_timestamp"/>
	...
	<xsl:template match="user">
		...
		<tr valign="middle">
			...
			<script><![CDATA[ent_name]]><xsl:value-of select="./@ent_id"/><![CDATA[=']]><xsl:call-template name="escape_js">
			...
			str += '<td>'
			str += '<span class="Text">'
			str += ']]><xsl:for-each select="roles/role">
