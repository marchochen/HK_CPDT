...
<!-- 	<xsl:variable name="_title" select="java:com.cw.wizbank.util.LangLabel.getValue('BS006')"/>  -->
	...
	<xsl:variable name="test_dsc" select="java:com.cwn.wizbank.utils.LabelContent.get($wb_cur_lang, 'test_dsc')"/>
	<xsl:variable name="test_fsc" select="java:com.cwn.wizbank.utils.LabelContent.get($wb_cur_lang, 'test_fsc')"/>
	<xsl:variable name="test_topic_desc" select="java:com.cwn.wizbank.utils.LabelContent.get($wb_cur_lang, 'test_topic_desc')"/>
	<xsl:variable name="units_points" select="java:com.cwn.wizbank.utils.LabelContent.get($wb_cur_lang, 'units_points')"/>
	...
	<xsl:template name="lang_ch">
		<xsl:call-template name="test_player">
			...
			<xsl:with-param name="lab_save_and_suspend">確認保存并退出？</xsl:with-param>
		</xsl:call-template>
	</xsl:template>
	...
	<xsl:template name="lang_gb">
		<xsl:call-template name="test_player">
			...
			<xsl:with-param name="lab_leave_prompt">你正在考试，如果此时离开，将保存本次考试次数，但不会记录本次考试结果。</xsl:with-param>
			<xsl:with-param name="lab_save_and_suspend">确认保存并退出？</xsl:with-param>
		</xsl:call-template>
	</xsl:template>
	...
	<xsl:template name="lang_en">
		<xsl:call-template name="test_player">
			...
			<xsl:with-param name="lab_examinee_id">User ID</xsl:with-param>
			<xsl:with-param name="lab_examinee_usg">试题名</xsl:with-param>
			...
			<xsl:with-param name="lab_leave_prompt">你正在考试，如果此时离开，将保存本次考试次数，但不会记录本次考试结果。</xsl:with-param>
			<xsl:with-param name="lab_save_and_suspend">Confirm save and exit?</xsl:with-param>
		</xsl:call-template>
	</xsl:template>
...
function SaveAndSuspendFunc() {
	saveTestData();
	if (submit_test) {
		if(confirm(']]><xsl:value-of select="$lab_save_and_suspend"/><![CDATA[')){
			ifload = true;
			parent.window.close();
		}
	}
...
function helpFunc(que_id, tkh_id) {
	...
	window.onbeforeunload = function(){
		...
		if(wb_utils_get_cookie("test_disabled_alert_tip") == "yes"){
	 		wb_utils_set_cookie("test_disabled_alert_tip","");
	 	}
		//submit_test:是否预览状态
    ...
    function changeflag1(){ifload = false; }
	]]>
	</script>
		...
		<body  leftmargin="0" marginwidth="0" marginheight="0" topmargin="0" onload="MM_initInteractions();setHidFlag();init();restoreMT();">
				...
				<div class="upper-info">
					...
					<xsl:choose>
						<xsl:when test="$quiz/header/@show_save_and_suspend_ind=1">
							<div class="upper-tit"  style="width: 666px;"><xsl:value-of select="$quiz/header/title"/></div>
						</xsl:when>
						<xsl:otherwise>
					...
					<div class="upper-tool">
				  		<input id="help_btn" name="wiztxt" class="upper-btn upper-blue margin-right10" type="button" value="{$lab_help}" onclick="javascript:helpFunc({/quiz/@id}, '{/quiz/aicc_data/@tkh_id}')"/>
				   		...
				   		<input name="wiztxt" class="upper-btn upper-orange submit-test" type="button" value="{$button_submit}" onclick="javascript:changeflag1();submitTest();"/>
					</div>
				</div>  
			</div>
			...
			<div class="area">
				<div class="area-info clearfix" id="over">
					...
					<div class="right fixedly" id="head_td" style="top:0px;">
						...
						<!-- 测验信息 -->
						<p class="margin-top15 margin-left10"><span class="margin-right15"><xsl:value-of select="$lab_questionnum"/>：</span><span id="total_que_value"></span></p>
						...
						<div class="pick">
							...
							<!-- 题号索引 --> 
							<div class="pick-box" style="max-height:206px;overflow-y:auto;">
								<xsl:for-each select="$quiz/question[body/interaction[@type!='DSC' and @type!='FSC']]">
									<a id="slot_td_idx{position()}" class="graycolor">
										<span class="UnSelectedQue" id="slot_idx{position()}" onclick="changeSlot('{position()}','flag_layer{position()}');changeflag();"><xsl:if test="position() &lt; 10">0</xsl:if><xsl:value-of select="position()"/></span>
									</a>
								</xsl:for-each>
								<xsl:for-each select="$quiz/question[header[@type='FSC' or @type='DSC']]/question_list/question">
									<a id="slot_td_idx{$fsc_start+position()}"  class="graycolor">
										<span class="UnSelectedQue" id="slot_idx{$fsc_start+position()}" onclick="changeSlot('{$fsc_start+position()}','flag_layer{$fsc_start+position()}');changeflag();"><xsl:if test="$fsc_start+position() &lt; 10">0</xsl:if><xsl:value-of select="$fsc_start+position()"/></span>
									</a>
								</xsl:for-each>
...
<!-- 					</xsl:for-each> -->
	...
	<xsl:template match="*" mode="TST">
		...
		<div class="wzb-bar">
			<input name="wiztxt" class="btn wzb-btn-blue wzb-btn-big submit-test" type="button" value="{$button_submit}" onclick="javascript:changeflag1();submitTest();"/>
		</div>
	</xsl:template>
	<!-- =============================================================== -->
...
<!-- 		<div id="layer{$_pos}_tmp"><img src="{$wb_img_path}tp.gif" height="30" width="1" border="0"/></div> -->
		<div id="layer{$_pos}" class="" style="z-index:90;" onmouseout="leaveQue({$_pos})">
					...
					<table width="{$que_title_width}" border="0" cellspacing="0" cellpadding="5">
						<tr height="30">
							<td align="left" valign="top">
								...
								<xsl:value-of select="$test_es"/>
								<xsl:text>（</xsl:text>
								<xsl:value-of select="$es_cnt"/>
								<xsl:value-of select="$test_topic_desc"/>
								<xsl:value-of select="$es_sum"/>
								<xsl:value-of select="$units_points"/>
								<xsl:text>）</xsl:text>
								</span>
							</td>
						</tr>
				...
				<dl class="list clearfix">
			   		<dt>
			   			<a href="javascript:;" id="flagStar{$_pos}" onclick="javascript:flagQue({$_pos});redrawSlotInd();">
			   				<i class="fa grayC999 fa-star" onclick="changeflag();"/>
			   			</a>
			   		</dt>
			     	<dd>
			       		<div class="list_tit">
							...
							<xsl:for-each select="body/interaction">
								<xsl:if test="$que_submit_file_ind = 'Y'">
									<script LANGUAGE="JavaScript" TYPE="text/javascript"><![CDATA[
																function helpTooltip_]]><xsl:value-of select="$que_id"/><![CDATA[() {
											var ht = document.getElementById('help_tooltip_'+]]><xsl:value-of select="$que_id"/> <![CDATA[);
									...
									<!-- 问答题屏蔽掉上传文件入口 -->
									<!-- <input class="btn wzb-btn-blue" type="button" style="padding:3px 8px;">
									...
									<a class="Text" href="javascript:;">
										<xsl:attribute name="onmouseover">helpTooltip_<xsl:value-of select="$que_id"/>();</xsl:attribute>
										<xsl:attribute name="onmouseout">helpTooltip_<xsl:value-of select="$que_id"/>();</xsl:attribute>
										<xsl:value-of select="$lab_help"/>
									</a>
									<div id="help_tooltip_{$que_id}" class="questipDesc" style="display: none; border: 1px solid #CCC; background: #EEE;padding: 3px;">
						...
						<xsl:for-each select="body/interaction"> <!--onBlur="leaveQue({$_pos})" 文本域失去焦点  验证是否填写内容  -->
							<textarea onBlur="leaveQue({$_pos})" class="wbpl showhide" style="margin:10px 0" name="txt_q{$_pos}_i{@order}" cols="70" rows="10" onChange="qset.getQuestion({$_pos}).getInteraction({@order}).setAnswer(this.value)" onKeyPress="qset.getQuestion({$_pos}).getInteraction({@order}).setAnswer(this.value);">
	...
	<xsl:template name="fb_que">
		...
		<xsl:comment>============ layer<xsl:value-of select="$_pos"/> ===========================================</xsl:comment>
<!-- 		<div id="layer{$_pos}_tmp"><img src="{$wb_img_path}tp.gif" height="30" width="1" border="0"/></div> -->
		<div id="layer{$_pos}" class="mainContent" style="z-index:90;" onmouseout="leaveQue({$_pos})">
				...
				<xsl:if test="position()=1">
					<table width="{$que_title_width}" border="0" cellspacing="0" cellpadding="5">
						<tr height="30">
							<td align="left" valign="top">
								<span class="TitleText">
									<xsl:value-of select="$test_fb"/>
									<xsl:text>（</xsl:text>
									<xsl:value-of select="$fb_cnt"/>
									<xsl:value-of select="$test_topic_desc"/>
									<xsl:value-of select="$fb_sum"/>
									<xsl:value-of select="$units_points"/>
									<xsl:text>）</xsl:text>
								</span>
							</td>
...
<!-- 		<div id="layer{$_pos}" class="mainContent ContentDiv" style="width:1000px; height:300px; z-index:{$_pos};"> -->
		<div id="layer{$_pos}" class="mainContent" style="z-index:90px;" onmouseout="leaveQue({$_pos})" ><!-- onmouseout="leaveQue({$_pos})" -->
			<form name="frm{$_pos}" onSubmit="return status()">
				<input type="hidden" name="module" value=""/>
				<input type="hidden" name="cmd" value=""/>
				<input type="hidden" name="url_failure" value=""/>
							...
							<input type="hidden" name = "flag_layer0"/>
								<xsl:choose>
									<xsl:when test="body/interaction/@logic='SINGLE'">
										<span class="TitleText">
											<xsl:value-of select="$test_mc_single"/>
											<xsl:text>（</xsl:text>
											<xsl:value-of select="$sg_mc_cnt"/>
											<xsl:value-of select="$test_topic_desc"/>
											<xsl:value-of select="$sg_mc_sum"/>
											<xsl:value-of select="$units_points"/>
											<xsl:text>）</xsl:text>
										</span>
									</xsl:when>
									<xsl:when test="body/interaction/@logic='AND' or body/interaction/@logic='OR'">
										<span class="TitleText">
											<xsl:value-of select="$test_mc_and"/>
											<xsl:text>（</xsl:text>
											<xsl:value-of select="$mt_mc_cnt"/>
											<xsl:value-of select="$test_topic_desc"/>
											<xsl:value-of select="$mt_mc_sum"/>
											<xsl:value-of select="$units_points"/>
											<xsl:text>）</xsl:text>
										</span>
									</xsl:when>
				...
				<dl class="list clearfix">
			   		<dt>
			   			<a href="javascript:;" id="flagStar{$_pos}" onclick="javascript:flagQue({$_pos});redrawSlotInd();">
			   				<i class="fa fa-star color-gray999" onclick="changeflag();"/>
			   			</a>
			   		</dt>
			     	<dd>
			       		<div class="list-tit">
						...
						<xsl:for-each select="body/interaction[@type='MC']/option">
							...
							<p class="list-info">
								<label for="rdo_q{$_pos}_i{@id}">
									<xsl:choose>
										<xsl:when test="../../../outcome/@logic='AND' or ../../../outcome/@logic='OR'">
											<input type="checkbox" id="rdo_q{$_pos}_i{@id}" class="xsval" name="rdo_q{$_pos}_i{$_order}" value="{@id}" onClick="qset.getQuestion({$_pos}).getInteraction({$_order}).setAnswer('');qset.getQuestion({$_pos}).getInteraction({$_order}).setAnswerShuffle('')">
										...
										<xsl:otherwise>
											<input type="radio" id="rdo_q{$_pos}_i{@id}" class="xsval" name="rdo_q{$_pos}_i{$_order}" value="{@id}" onClick="qset.getQuestion({$_pos}).getInteraction({$_order}).setAnswer(this.value);qset.getQuestion({$_pos}).getInteraction({$_order}).setAnswerShuffle({$optionPos})">
				...
				<xsl:if test="position()=1">
					<table width="{$que_title_width}" border="0" cellspacing="0" cellpadding="5">
						<tr height="30">
							<td align="left" valign="top">
								<span class="TitleText">
									<xsl:value-of select="$test_tf"/>
									<xsl:text>（</xsl:text>
									<xsl:value-of select="$tf_cnt"/>
									<xsl:value-of select="$test_topic_desc"/>
									<xsl:value-of select="$tf_sum"/>
									<xsl:value-of select="$units_points"/>
									<xsl:text>）</xsl:text>
								</span>
							</td>
	...
	<xsl:template name="mt_que">
		...
		<xsl:comment>============ layer<xsl:value-of select="$_pos"/> ===========================================</xsl:comment>
		<div align="left" id="layer{$_pos}" class="mainContent" style="overflow:visible;z-index:90;" onmouseout="leaveQue({$_pos})">
			...
			<xsl:if test="position()=1">
				<table width="{$que_title_width}" border="0" cellspacing="0" cellpadding="5">
					<tr height="30">
						<td align="left" valign="top">
							<span class="TitleText">
								<xsl:value-of select="$test_mt"/>
								<xsl:text>（</xsl:text>
								<xsl:value-of select="$mt_cnt"/>
								<xsl:value-of select="$test_topic_desc"/>
								<xsl:value-of select="$mt_sum"/>
								<xsl:value-of select="$units_points"/>
								<xsl:text>）</xsl:text>
							</span>
						</td>
					...
					<div id="G0{$_pos}OptionTable" style="position:relative;height:{$parent_height}px;overflow:auto;float:left;width:400px;" onclick="showHideDragedDiv(event,{$_pos})" onmousemove="changeDragedPosition(event,{$_pos})">
						<span style="width:170px;float:left;height:{$parent_height}px;margin:0 0 10px 0;">
							...
							<xsl:for-each select="body/source/item">
								...
								<p class="dcmess" onmouseover="overOption('source', {$item_id}, {$_pos})" onmouseout="outOption('source', {$item_id}, {$_pos})" style="height:auto;">
									...
									<span id="G0{$_pos}sourceText_{$item_id}" style="line-height: 15px;">
						...
						<span style="border-left:1px dashed gray;width:200px;float:left;padding:0 0 0 10px;">
							<xsl:for-each select="body/interaction">
								<xsl:variable name="int_ord" select="@order"/>
								<p class="dcmess" style="padding-left:3px;height:auto;" onmouseover="overOption('target', {$int_ord}, {$_pos})" onmouseout="outOption('target', {$int_ord}, {$_pos})">
									<xsl:if test="count(object)!=0">
									...
									<span id="G0{$_pos}targetText_{$int_ord}" style="line-height: 15px;">
										<xsl:value-of select="./text"/>
									</span>
									<xsl:if test="count(object)!=0">
										<br/>
...
<!-- 					</xsl:call-template> -->
					<a href="javascript:;" onclick="javascript:changeflag();deleteOneOption({$_pos}, this);">
							...
							<td align="left" valign="top">
								<xsl:choose>
									<xsl:when test="../../header/@type = 'DSC'">
										<span class="TitleText">
											<xsl:value-of select="$test_dsc"/>
											<xsl:text>（</xsl:text>
											<xsl:value-of select="$dsc_cnt"/>
											<xsl:value-of select="$test_topic_desc"/>
											<xsl:value-of select="$dsc_sum"/>
											<xsl:value-of select="$units_points"/>
											<xsl:text>）</xsl:text>
										</span>
									</xsl:when>
									<xsl:when test="../../header/@type = 'FSC'">
										<span class="TitleText">
											<xsl:value-of select="$test_fsc"/>
											<xsl:text>（</xsl:text>
											<xsl:value-of select="$fsc_cnt"/>
											<xsl:value-of select="$test_topic_desc"/>
											<xsl:value-of select="$fsc_sum"/>
											<xsl:value-of select="$units_points"/>
											<xsl:text>）</xsl:text>
										</span>
									</xsl:when>
					...
					<dd>
						...
						<xsl:for-each select="body/interaction[@type='MC']/option">
							...
							<p class="dcmess">
								<xsl:choose>
									<xsl:when test="../../../outcome/@logic='AND' or ../../../outcome/@logic='OR'">
										<input type="checkbox" class="xsval" name="rdo_q{$_pos}_i{$_order}" value="{@id}" onClick="qset.getQuestion({$_pos}).getInteraction({$_order}).setAnswer('');qset.getQuestion({$_pos}).getInteraction({$_order}).setAnswerShuffle('')">
											<xsl:for-each select="../../../../restore/question[@id=$que_id]/interaction/value">
												<xsl:if test="$optionId=text()">
													<xsl:attribute name="checked">checked</xsl:attribute>
												</xsl:if>
									...
									<xsl:otherwise>
										<input type="radio" id="{@id}" class="xsval" name="rdo_q{$_pos}_i{$_order}" value="{@id}" onClick="qset.getQuestion({$_pos}).getInteraction({$_order}).setAnswer(this.value);qset.getQuestion({$_pos}).getInteraction({$_order}).setAnswerShuffle({$optionPos})">
											<xsl:if test="$optionId=../../../../restore/question[@id=$que_id]/interaction/value">
												<xsl:attribute name="checked">checked</xsl:attribute>
											</xsl:if>
										</input>
