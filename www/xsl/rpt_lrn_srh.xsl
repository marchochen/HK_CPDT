...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template name="main">
		<head>
			...
			function usr_change(frm, obj){
					...
					if(frm.ent_id_lst.type == 'select-multiple'){
						frm.ent_id_lst.disabled = pos;
						if(frm.genaddent_id_lst){
							frm.genaddent_id_lst.disabled = pos;
						}
						if(frm.genremoveent_id_lst){
							frm.genremoveent_id_lst.disabled = pos;
						}		
						if(frm.gensearchent_id_lst){
							frm.gensearchent_id_lst.disabled = pos;
						}		
						if(pos == true){
							frm.ent_id_lst.options.length = 0
						}
					}
	...
	<xsl:template name="lang_ch">
		<xsl:apply-templates select="report_body">
			...
			<xsl:with-param name="lab_item_page">*每頁記錄數</xsl:with-param>
			...
			<xsl:with-param name="lab_ats_id_lst">考勤狀況</xsl:with-param>
	...
	<xsl:template name="lang_gb">
		<xsl:apply-templates select="report_body">
			...
			<xsl:with-param name="lab_select_all">-- 全部 --</xsl:with-param>
			<xsl:with-param name="lab_ats_id_lst">考勤状态</xsl:with-param>
			...
			<xsl:with-param name="lab_att_rate">
				...
				<xsl:with-param name="lab_item_page">*每页记录数</xsl:with-param>
			<xsl:with-param name="lab_att_remark">考勤備註</xsl:with-param>
	...
	<xsl:template name="lang_en">
		<xsl:apply-templates select="report_body">
			...
			<xsl:with-param name="lab_item_page">*Items per page</xsl:with-param>
			<!-- content attribute text -->
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:apply-templates select="display_option">
						...
						<xsl:with-param name="lab_others_content" select="$lab_others_content"/>
						...
						<!--<xsl:with-param name="lab_cost_center" select="$lab_cost_center"/>-->
	...
	<xsl:template match="spec">
					...
					<tr>
						<td valign="top">
							<span style="padding-right:0px;">
								<input type="radio" name="usr_sel_all_user" value="{$root_ent_id}" id="rdo_sel_by_all_user" onclick="usr_change(document.frmXml,this);changelrn_cos_ContentStatus(false,'lrn')">
						...
						<td valign="top">
							<span style="padding-right:0px;">
								<input type="radio" name="usr_sel_all_user" value="0" id="rdo_sel_by_selected_user" onclick="usr_change(document.frmXml,this);changelrn_cos_ContentStatus(true,'lrn')">
									<xsl:if test="not($report_body/spec/data_list/data[@name='all_user_ind']/@value = '1') and ($report_body/spec/data_list/data[@name='all_user_ind']/@value)">
										<xsl:attribute name="checked">checked</xsl:attribute>
									</xsl:if>
								</input>
							</span>
						...
						<td>
							<xsl:call-template name="wb_goldenman">
								...
								<xsl:with-param name="option_list">
									<xsl:apply-templates select="$data_list/data[@name='ent_id']"/>
								</xsl:with-param>
		...
		<!-- Attendance ID list 
							<xsl:variable name="progress_ats_id">
						<xsl:value-of select="/report/report_body/meta/attendance_status_list/status[@type='PROGRESS']/@id"/>
					</xsl:variable>
		<xsl:variable name="ats_id_lst">
			<xsl:for-each select="/report/report_body/meta/attendance_status_list/status">
				<xsl:value-of select="@id"/>
				<xsl:text>~</xsl:text>
			</xsl:for-each>
		</xsl:variable>
		<input type="hidden" name="ats_id_lst" value="{$ats_id_lst}"/>
		<input type="hidden" name="ats_id_progress" value="">
					</input>
		<tr>
			...
			<td width="80%">
				<span class="Text">
					...
					<xsl:variable name="ats_id_lst">
						<xsl:for-each select="/report/report_body/meta/attendance_status_list/status[@id != $progress_ats_id]">
							<xsl:value-of select="@id"/>
							<xsl:text>~</xsl:text>
						</xsl:for-each>
					</xsl:variable>
					<input type="hidden" name="ats_id_lst" value="{$ats_id_lst}"/>
					<input type="checkbox" name="ats_id_progress" value="{$progress_ats_id}">
						<xsl:choose>
							<xsl:when test="not(data_list)"><xsl:attribute name="checked">checked</xsl:attribute></xsl:when>
							<xsl:when test="data_list/data[@name='ats_id' and @value=$progress_ats_id]"><xsl:attribute name="checked">checked</xsl:attribute></xsl:when>
						</xsl:choose>
					</input>
					<xsl:copy-of select="$lab_show_in_progress_attendance"/>
				</span>
			</td>
		</tr>-->
		<!--== Period ==-->
		...
		<tr>
			...
			<td width="80%" class="wzb-form-control">
				<label for="ats_id_all" style="margin-left:3px;">
				...
				<xsl:for-each select="/report/report_body/meta/attendance_status_list/status">
				 <xsl:text>&#160;</xsl:text>
					...
					<!--<xsl:choose>
			                    <xsl:when test="$cs='1'">
			                            <input type="radio" name="ats_id" value="cs"><xsl:value-of select="desc[@lan='ISO-8859-1']/                  @name"/></input>
			                      </xsl:when>     
			                       <xsl:when test="$cs='2'">
			                            <input type="radio" name="ats_id" value="cs"><xsl:value-of select="desc[@lan='ISO-8859-1']/                    @name"/></input>
                                           </xsl:when>
	...
	<xsl:template match="spec" mode="sort">
		...
		<tr>
			...
			<td width="20%" class="wzb-form-control">
				...
				<select name="sort_col" class="select">
					<!-- user中的字段-->
					<xsl:for-each select="/report/report_body/display_option/user/object_view/attribute">
						<xsl:variable name="curr" select="."/>
						<xsl:if test="@sortable='yes'">
						   <option>
						     <xsl:choose>
									 <xsl:when test="@paramname">
									     <xsl:variable name="param" select="@paramname" />
									     <xsl:attribute name="value"><xsl:value-of select="$param" /></xsl:attribute>
									     <xsl:if test="$param=$sort_value">
									         <xsl:attribute name="selected">selected</xsl:attribute>
									     </xsl:if>
									 </xsl:when>
									 <xsl:otherwise>
									     <xsl:attribute name="value"><xsl:value-of select="$curr" /></xsl:attribute>
									     <xsl:if test="$curr=$sort_value">
									         <xsl:attribute name="selected">selected</xsl:attribute>
									     </xsl:if>
									 </xsl:otherwise>
								</xsl:choose>
						      <xsl:choose>
											<xsl:when test="$curr='usr_id'">
												<xsl:value-of select="$lab_login_id"/>
											</xsl:when>
											<xsl:when test="$curr='usr_display_bil'">
					...
					<!-- other中的字段-->
					<xsl:for-each select="/report/report_body/display_option/other/object_view/attribute">
						<xsl:variable name="current" select="."/>
						<xsl:if test="@sortable='yes'">
							<option value="{.}">
							    <xsl:if test=".=$sort_value">
							        <xsl:attribute name="selected">selected</xsl:attribute>
							    </xsl:if>
							    <xsl:choose>
											<xsl:when test="$current='att_timestamp'">
												<xsl:value-of select="$lab_att_date"/>
											</xsl:when>
											<xsl:when test="$current='att_create_timestamp'">
     ...
     <xsl:for-each select="/report/report_body                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          /display_option/user/object_view/attribute[@sortable='yes']">
      ...
      <xsl:choose>
				<xsl:when test="@paramname"><xsl:value-of select="concat($name1,'_display')" />              </xsl:when>
                    <xsl:otherwise><xsl:value-of select="concat($name2,'_display')" />          </xsl:otherwise>
		</xsl:choose>      
		</xsl:attribute>
       <xsl:attribute name="value">
        <xsl:choose>
         <xsl:when test=". = 'usr_id'"><xsl:value-of select="$lab_login_id"/></xsl:when>
         <xsl:when test=". = 'usr_display_bil'"><xsl:value-of select="$lab_dis_name"/></xsl:when>
        </xsl:choose>
       </xsl:attribute> 
      </input>
      <!--=====另一个hidden域======-->
      <xsl:choose>
				<xsl:when test="@paramname"><input type="hidden" name="{@paramname}_fieldname" value="{text()}"/></xsl:when>
                    <xsl:otherwise><input type="hidden" name="{text()}_fieldname" value="{text()}"/></xsl:otherwise>
		</xsl:choose>
     ...
     <!-- other中的字段-->
     <xsl:for-each select="/report/report_body/display_option/other/object_view/attribute[@sortable='yes']">
      <input type="hidden" name="{text()}_display">
       <xsl:attribute name="value">
        <xsl:choose>
         <xsl:when test=". = 'att_timestamp'"><xsl:value-of select="$lab_att_date"/></xsl:when>
         <xsl:when test=". = 'att_create_timestamp'"><xsl:value-of select="$lab_att_create_timestamp"/>    </xsl:when>
        </xsl:choose>
       </xsl:attribute> 
      </input>
      <input type="hidden" name="{text()}_fieldname" value="{name()}"/>
     </xsl:for-each>
    <!-- ================================DENNIS END============================================= -->
                ...
                <xsl:value-of select="$lab_sort_ordera" /></option>
					<option value="desc">
						<xsl:if test="$index = 'desc'">
							<xsl:attribute name="selected">selected</xsl:attribute>
						</xsl:if>
               <xsl:value-of select="$lab_sort_orderd" /></option>
				</select>
			</td>
		</tr>
		<!-- 
		<tr>
			<td width="20%" align="right">
				<span class="TitleText">
					<xsl:value-of select="$lab_item_page"/>
					<xsl:text>：</xsl:text>
				</span>
			</td>
			<td width="80%">
				<select name="page_size" class="select">
					<option value="10">
						<xsl:if test="$itemno = '10'">
							<xsl:attribute name="selected">selected</xsl:attribute>
						</xsl:if>
                10</option>
					<option value="20">
						<xsl:if test="$itemno = '20'">
							<xsl:attribute name="selected">selected</xsl:attribute>
						</xsl:if>20</option>
					<option value="50">
						<xsl:if test="$itemno = '50'">
							<xsl:attribute name="selected">selected</xsl:attribute>
						</xsl:if>50</option>
					<option value="100">
						<xsl:if test="$itemno = '100'">
							<xsl:attribute name="selected">selected</xsl:attribute>
						</xsl:if>100</option>	
				</select>
			</td>
	...
	<xsl:template match="item" mode="content">
		...
		<table cellpadding="0" cellspacing="0" width="100%" border="0">
			<tr>
				<td colspan="3">
					<span class="Text">
						<xsl:value-of select="$lab_item_content"/>：</span>
				</td>
			</tr>
			<xsl:call-template name="template_view">
				<xsl:with-param name="type">itm</xsl:with-param>
			</xsl:call-template>
		</table>
	</xsl:template>
	<!-- =============================================================== -->
				...
				<td valign="top">
					...
					<span class="Text">
						<xsl:choose>
							...
							<xsl:when test="name() ='training_center'">
								  <xsl:if test=" $tc_enabled ='true'">
