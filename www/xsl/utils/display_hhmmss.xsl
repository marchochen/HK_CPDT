...
<!-- Display HH:MM:SS of Timestamp -->
<xsl:template name="display_hhmmss">
	...
	<xsl:variable name="my_secs" select="substring($tmp_timestamp,13,2)"/>
	...
	<xsl:choose>
		...
		<xsl:when test="$mode = 'js'">
			<script language="javascript"><![CDATA[
				...
				time_str += ']]><xsl:value-of select="$my_secs"/><![CDATA['
