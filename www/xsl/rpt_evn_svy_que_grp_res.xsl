...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="lang_ch">
		<xsl:call-template name="content">
			...
			<xsl:with-param name="lab_eva_survey">公共調查問卷</xsl:with-param>
			...
			<xsl:with-param name="lab_tol_enr">發佈總數</xsl:with-param>
			...
			<xsl:with-param name="lab_view">請使用導出功能顯示全部數據</xsl:with-param>
	...
	<xsl:template name="lang_gb">
		<xsl:call-template name="content">
			...
			<xsl:with-param name="lab_all_cos">所有課程</xsl:with-param>
			<xsl:with-param name="lab_eva_survey">公共调查问卷</xsl:with-param>
			<xsl:with-param name="lab_enr_period">录取日期</xsl:with-param>
			<xsl:with-param name="lab_from">由</xsl:with-param>
			<xsl:with-param name="lab_to">至</xsl:with-param>
			<xsl:with-param name="lab_rep_sum">报告摘要</xsl:with-param>
			<xsl:with-param name="lab_tol_enr">发布总数</xsl:with-param>
			...
			<xsl:with-param name="lab_answer">回答</xsl:with-param>
			<xsl:with-param name="lab_view">请使用导出功能显示全部数据</xsl:with-param>
			<xsl:with-param name="lab_html_content">HTML内容</xsl:with-param>
	...
	<xsl:template name="lang_en">
		<xsl:call-template name="content">
			...
			<xsl:with-param name="lab_eva_survey">Survey form</xsl:with-param>
			...
			<xsl:with-param name="lab_tol_enr">Total targeted</xsl:with-param>
			...
			<xsl:with-param name="lab_view">Please export the data to show all</xsl:with-param>
...
function initSummary() {
		...
		<xsl:apply-templates select="report_head">
			...
			<xsl:with-param name="lab_eva_survey" select="$lab_eva_survey"/>
	...
	<xsl:template match="que_stat_list">
								...
								<a href="#que_{@order}">
									<xsl:choose>
										<xsl:when test="body/html">
											<span class="Text">
												<xsl:value-of select="body/html" disable-output-escaping="yes"></xsl:value-of>
											</span>
										</xsl:when>
										<xsl:otherwise>
											<xsl:choose>
												<xsl:when test="string-length(body/text()) &gt; 23">
													<xsl:value-of select="body/text()" disable-output-escaping="yes"></xsl:value-of>
												</xsl:when>
												<xsl:otherwise>
		...
		<!-- detail -->
		<table cellspacing="0" cellpadding="0" border="0" width="{$wb_gen_table_width}" class="table wzb-ui-table">
			<xsl:for-each select="que_stat">
				<xsl:choose>
					...
					<tr class="{$row_class}">
						<td>
							<span class="Text">
								...
								<xsl:value-of select="../../body/interaction/option[@id = $cur_opt_id]" disable-output-escaping="yes"></xsl:value-of>
							</span>
						</td>
						<td align="right">
							<span class="Text">
	...
	<xsl:template match="report_head">
				...
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:call-template name="get_rte_title">
							<xsl:with-param name="rte_type">SURVEY_IND</xsl:with-param>
						</xsl:call-template>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:with-param>
		...
		<table cellpadding="3" cellspacing="0" border="0" width="{$wb_gen_table_width}" class="Bg">
			...
			<tr>
				<!-- <td width="200" align="right" valign="top" class="wzb-form-label">
					<span class="TitleText">
						<xsl:value-of select="$lab_eva_survey"/>：</span>
				</td> -->
				<td valign="top" class="wzb-form-control">
					<span class="Text">
