	...
	<xsl:template name="content">
		...
		<div class="wzb-bar">
				<xsl:call-template name="wb_gen_form_button">
					<xsl:with-param name="wb_gen_btn_name" select="$label_core_training_management_67"></xsl:with-param>
					<xsl:with-param name="wb_gen_btn_href">javascript:itm_lst.ins_simple_item_exec(document.frmXml,'<xsl:value-of select="$itm_type_lst"/>')</xsl:with-param>
				</xsl:call-template>
