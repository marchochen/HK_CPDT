...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="lang_ch">
		<xsl:apply-templates>
			<xsl:with-param name="lab_enrollment_approval">
				<xsl:value-of select="$lab_const_enrollment"/>審批</xsl:with-param>
			<xsl:with-param name="lab_enrollment_approval_desc">以下是需要你審批的報名請求。 要批准報名請求，點擊 <b>"審批"</b>； 要拒絕報名請求，點擊 <b>"拒絕"</b> 並輸入拒絕的原因。</xsl:with-param>
	...
	<xsl:template name="lang_gb">
		<xsl:apply-templates>
			<xsl:with-param name="lab_enrollment_approval">
				<xsl:value-of select="$lab_const_enrollment"/>审批</xsl:with-param>
			<xsl:with-param name="lab_enrollment_approval_desc">以下是需要您审批的报名请求。 要批准报名请求，点击 <b>"审批"</b>； 要拒绝报名请求，点击 <b>"拒绝"</b> 并输入拒绝的原因。</xsl:with-param>
			<xsl:with-param name="lab_enrollment_details">
	...
	<xsl:template name="lang_en">
		<xsl:apply-templates>
			...
			<xsl:with-param name="lab_enrollment_approval_desc">Listed below is the information of the enrollment pending for your approval. Click <b>审批</b> to approve the enrollment. If you decide to decline the enrollment, click <b>拒绝</b> and you will be asked to provide the decline reason.</xsl:with-param>
			<xsl:with-param name="lab_enrollment_details">
	...
	<xsl:template match="applyeasy">
		...
		<xsl:param name="lab_enrollment_approval_desc"/>
		...
		<xsl:call-template name="wb_ui_hdr">
			...
			<xsl:with-param name="parent_code">FUN_APPR_APP_LIST</xsl:with-param>
		</xsl:call-template>
		<xsl:call-template name="wb_ui_title">
			<xsl:with-param name="text" select="$lab_enrollment_details"/>
		</xsl:call-template>
		<xsl:call-template name="wb_ui_desc">
			<xsl:with-param name="text" select="$lab_enrollment_approval_desc"/>
		</xsl:call-template>
			...
			<tr>
				<td align="center">
					<xsl:call-template name="wb_gen_form_button">
						...
						<xsl:with-param name="wb_gen_btn_href">javascript:app.enrol_approval.get_my_appn_approval_list('PENDING')</xsl:with-param>
					</xsl:call-template>
				</td>
			</tr>
		</table>
