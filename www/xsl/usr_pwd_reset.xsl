...
<xsl:template name="lang_ch">
	<xsl:call-template name="content">
		<xsl:with-param name="lab_reset_pwd">重新設定<xsl:value-of select="$lab_passwd"/></xsl:with-param>	
		<xsl:with-param name="lab_pwd_info"><xsl:value-of select="$lab_passwd"/>資料</xsl:with-param>
		<xsl:with-param name="lab_pwd_desc">請輸入該用戶的新<xsl:value-of select="$lab_passwd"/>。</xsl:with-param>
...
<xsl:template name="lang_gb">
	<xsl:call-template name="content">
		<xsl:with-param name="lab_reset_pwd">重新设定<xsl:value-of select="$lab_passwd"/></xsl:with-param>	
		<xsl:with-param name="lab_pwd_info"><xsl:value-of select="$lab_passwd"/>资料</xsl:with-param>
		<xsl:with-param name="lab_pwd_desc">请输入该用户的新<xsl:value-of select="$lab_passwd"/>。</xsl:with-param>
...
<xsl:template name="lang_en">
	<xsl:call-template name="content">
		<xsl:with-param name="lab_reset_pwd">Reset password</xsl:with-param>	
		<xsl:with-param name="lab_pwd_info"><xsl:value-of select="$lab_passwd"/> information</xsl:with-param>
		<xsl:with-param name="lab_pwd_desc">Enter the user's new password.</xsl:with-param>
	...
	<body leftmargin="0" marginwidth="0" marginheight="0" topmargin="0" onload="wb_utils_gen_form_focus(document.frmXml)">
		<form name="frmXml">			
			...
			<xsl:call-template name="wb_ui_title">
				<xsl:with-param name="text" select="$lab_reset_pwd"/>
			</xsl:call-template>
			...
			<div class="wzb-bar">
				<xsl:call-template name="wb_gen_form_button">
					...
					<xsl:with-param name="wb_gen_btn_href">javascript:usr.user.reset_pwd_exec(document.frmXml,'<xsl:value-of select="$wb_lang"/>')</xsl:with-param>
				</xsl:call-template>
				<xsl:call-template name="wb_gen_form_button">
