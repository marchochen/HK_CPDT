...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:variable name="module_cnt" select="count(/report/report_body/meta/module_list/module)"/>
					...
					if(frm.itm_type_lst.length){
						for(i=0;i<frm.itm_type_lst.length;i++){
							if(frm.itm_type_lst[i].type == 'checkbox'){
								frm.itm_type_lst[i].disabled = pos;
								if(pos == true){
									frm.itm_type_lst[i].checked = false;
								}else{
									frm.itm_type_lst[i].checked = true;
								}
							}
						}
					}else{
						if(frm.itm_type_lst.type == 'checkbox'){
							frm.itm_type_lst.disabled = pos;
							if(pos == true){
								frm.itm_type_lst.checked = false;
							}								
						}
					}	
	...
	<xsl:template name="lang_ch">
		<xsl:apply-templates select="report_body">
			...
			<xsl:with-param name="lab_classroom">教室</xsl:with-param>
			...
			<xsl:with-param name="lab_inst_info">請指定查詢標準:</xsl:with-param>
			...
			<xsl:with-param name="lab_report_column">報告欄</xsl:with-param>
	...
	<xsl:template name="lang_gb">
		<xsl:apply-templates select="report_body">
			...
			<xsl:with-param name="lab_inst_info">请指定查询标准：</xsl:with-param>
			...
			<xsl:with-param name="lab_report_column">报告栏</xsl:with-param>
	...
	<xsl:template name="lang_en">
		<xsl:apply-templates select="report_body">
			...
			<xsl:with-param name="lab_classroom">Classroom</xsl:with-param>
			...
			<xsl:with-param name="lab_report_column">Report columns</xsl:with-param>
	...
	<xsl:template match="report_body">
		...
		<xsl:param name="lab_report_column"/>
			...
			<xsl:otherwise>
				<table cellpadding="0" cellspacing="0" border="0" width="{$wb_gen_table_width}" class="Bg">
					...
					<xsl:apply-templates select="spec">
						...
						<xsl:with-param name="lab_classroom" select="$lab_classroom"/>
					...
					<xsl:choose>
						<xsl:when test="$spec_ent_id = '0'  and $spec_id!='0'">
							<!-- for standard report : draw hidden field-->
							<xsl:apply-templates select="display_option" mode="hidden"/>
						</xsl:when>
						<xsl:otherwise>
							<!-- for personal report : draw check box -->
							<xsl:apply-templates select="display_option">
								<xsl:with-param name="lab_report_column" select="$lab_report_column"/>
								<xsl:with-param name="lab_item_info" select="$lab_item_info"/>
								...
								<xsl:with-param name="lab_question_rating" select="$lab_question_rating"/>
						...
						<xsl:otherwise>
							<tr>
								<td valign="top" align="right">
									<input type="radio" name="course_sel_type" value="1" id="rdo_sel_by_type_or_cata" onclick="course_change(document.frmXml,this)">
	...
	<xsl:template match="display_option">
		...
		<xsl:param name="lab_overall_rating"/>
		<tr>
			<td align="right" valign="top" width="20%" class="wzb-form-label">
					<span class="TitleText">
						<xsl:value-of select="$lab_report_column"/>
						<xsl:text>：</xsl:text>
					</span>
			</td>
			<td width="80%" class="wzb-form-control">
	...
	<xsl:template match="display_option" mode="hidden">
		<xsl:apply-templates mode="hidden_content"/>
	</xsl:template>
	<!-- =============================================================== -->
	<!-- Item -->
	<xsl:template match="item" mode="hidden_content">
		<input type="hidden" name="hidden_spec_name" value="itm_content_lst"/>
		<input type="hidden" name="hidden_spec_value">
			<xsl:attribute name="value"><xsl:for-each select="$data_list/data[@name = 'itm_content_lst']"><xsl:value-of select="@value"/><xsl:if test="position()!= last()">~</xsl:if></xsl:for-each></xsl:attribute>
		</input>
	</xsl:template>
	<xsl:template match="item" mode="content">
		...
		<table cellpadding="0" cellspacing="0" width="100%" border="0">
			<tr>
				<td colspan="3">
					<span class="TitleText">
						<xsl:value-of select="$lab_item_info"/>：
					</span>
				</td>
			</tr>
	...
	<!-- =============================================================== -->
	<!-- Run	-->
	<xsl:template match="run" mode="hidden_content">
		<input type="hidden" name="hidden_spec_name" value="run_content_lst"/>
		<input type="hidden" name="hidden_spec_value">
			<xsl:attribute name="value"><xsl:for-each select="$data_list/data[@name = 'run_content_lst']"><xsl:value-of select="@value"/><xsl:if test="position()!= last()">~</xsl:if></xsl:for-each></xsl:attribute>
		</input>
	</xsl:template>
					...
					<span class="Text">
						<xsl:choose>
							<xsl:when test="name() = 'item_access' or name() = 'run_item_access' or name() = 'child_item_access' ">
								...
								<label for="label_{name()}_{@id}">
									<input type="checkbox" name="{$type}_content" value="{name()}_{@id}" id="label_{name()}_{@id}">
										<xsl:if test="@checked = 'yes' and $spec_id = 0">
											<xsl:attribute name="checked">checked</xsl:attribute>
										</xsl:if>
							...
							<xsl:otherwise>
								<xsl:variable name="_my_name" select="name()"/>
								<label for="label_{name()}">
									<input type="checkbox" name="{$type}_content" value="{name()}" id="label_{name()}">
										<xsl:if test="@checked = 'yes' and $spec_id = 0">
											<xsl:attribute name="checked">checked</xsl:attribute>
										</xsl:if>
										<xsl:if test="$data_list/data[@value = $_my_name]">
	...
	<!-- =============================================================== -->
	<!-- Others -->
	<xsl:template match="other" mode="hidden_content">
		<input type="hidden" name="hidden_spec_name" value="content_lst"/>
		<input type="hidden" name="hidden_spec_value">
			<xsl:attribute name="value"><xsl:for-each select="$data_list/data[@name = 'content_lst']"><xsl:value-of select="@value"/><xsl:if test="position()!= last()">~</xsl:if></xsl:for-each></xsl:attribute>
		</input>
	</xsl:template>
	<!-- =============================================================== -->	
	<xsl:template match="other" mode="content">
		...
		<tr>
			<td align="right" valign="top" width="20%" class="wzb-form-label">
					<span class="TitleText">
						<xsl:value-of select="$lab_others_content"/>
						<xsl:text>：</xsl:text>
					</span>
			</td>
			<td width="80%" class="wzb-form-control">
								...
								<span class="Text">
									<label for="label_{.}">
										<input type="checkbox" name="content" value="{.}" id="label_{.}">
											<xsl:choose>
												<xsl:when test="not($report_body/spec/data_list) and . = 'question'">
													<xsl:attribute name="checked">checked</xsl:attribute>
												</xsl:when>
												<xsl:when test="$data_list/data[@value = $_my_name]">
													<xsl:attribute name="checked">checked</xsl:attribute>
												</xsl:when>
											</xsl:choose>
										</input>
