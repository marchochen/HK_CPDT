...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template name="main">
		<head>
			...
			function changeContentStatus(status) {
				frm = document.frmXml;
				var ctn = [frm.itm_content, frm.content];
	...
	<xsl:template name="lang_ch">
		<xsl:apply-templates select="report_body">
			...
			<xsl:with-param name="lab_rpt_name_length">（不可超過80字元）</xsl:with-param>
			...
			<xsl:with-param name="lab_show_learner_only">只顯示學員統計摘要</xsl:with-param>
			<xsl:with-param name="lab_show_all">顯示學員統計摘要以及包含以下信息的學習記錄</xsl:with-param>
			...
			<xsl:with-param name="lab_columns_info">每個學員分組顯示以下信息</xsl:with-param>
			<!-- button text -->
	...
	<xsl:template name="lang_gb">
		<xsl:apply-templates select="report_body">
			...
			<xsl:with-param name="lab_yes">是</xsl:with-param>
			<xsl:with-param name="lab_no">否</xsl:with-param>
			<xsl:with-param name="lab_show_learner_only">只显示学员统计摘要</xsl:with-param>
			<xsl:with-param name="lab_show_all">显示学员统计摘要以及包含以下信息的学习记录</xsl:with-param>
			...
			<xsl:with-param name="lab_columns_info">每个学员分组显示以下信息</xsl:with-param>
			<!-- 排序的顺序-->
	...
	<xsl:template name="lang_en">
		<xsl:apply-templates select="report_body">
			...
			<xsl:with-param name="lab_show_learner_only">Show learner summary statistics only</xsl:with-param>
			<xsl:with-param name="lab_show_all">Show learner summary statistics and enrollment details with the following information</xsl:with-param>
			...
			<xsl:with-param name="lab_columns_info">Group by learner showing the following information</xsl:with-param>
			<!-- button text -->
	...
	<xsl:template match="report_body">
		...
		<xsl:param name="lab_show_learner_only"/>
		...
		<table cellpadding="0" cellspacing="0" border="0" width="{$wb_gen_table_width}" class="Bg">
			<xsl:apply-templates select="spec">
				...
				<xsl:with-param name="lab_show_learner_only" select="$lab_show_learner_only"/>
	...
	<xsl:template match="spec">
		...
		<!--== Show data without record ==-->
		<tr>
			<td align="right" width="20%" class="wzb-form-label">
				<span class="wzb-form-star">*</span>
				<span class="TitleText">
					<xsl:value-of select="$lab_include_no_record_learner" />
				</span>
				<xsl:text>：</xsl:text>
			</td>
			<td width="80%" class="wzb-form-control">
				<input type="radio" name="include_no_record" value="true" id="show_empty_learner" style="margin-left:3px;">
					...
					<label for="show_empty_learner"><xsl:value-of select="$lab_yes"/></label>
				</input>
				<input type="radio" name="include_no_record" value="false" id="not_show_empty_learner" style="margin-left:15px;">
					<xsl:if test="/report/report_body/spec/data_list/data[@name='include_no_record' and @value='false']">
						<xsl:attribute name="checked">checked</xsl:attribute>
					</xsl:if>
					<label for="not_show_empty_learner"><xsl:value-of select="$lab_no"/></label>
				</input>
			</td>
		</tr>
		<!--== Catalog ==-->
			...
			<td width="20%" class="wzb-form-control">
				...
				<xsl:for-each select="/report/report_body/display_option/other/object_view/attribute[@sortable='yes']">
					<input type="hidden" name="{text()}_display">
						<xsl:attribute name="value"><xsl:choose><xsl:when test=". = 'att_timestamp'"><xsl:value-of select="$lab_att_date"/></xsl:when><xsl:when test=". = 'att_create_timestamp'"><xsl:value-of select="$lab_att_create_timestamp"/></xsl:when></xsl:choose></xsl:attribute>
					</input>
	...
	<xsl:template match="section ">
		...
		<xsl:for-each select="*[@sortable='yes']">s
		...
		<table cellpadding="0" cellspacing="0" width="100%" border="0">
			...
			<xsl:for-each select="template_view/section/*">
				<input type="hidden">
					...
					<xsl:attribute name="value"><xsl:value-of select="title/desc[@lan=$wb_lang_encoding]/@name"/></xsl:attribute>
				</input>
			</xsl:for-each>
		</table>
		<br/>
