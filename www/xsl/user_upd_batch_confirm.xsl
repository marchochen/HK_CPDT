...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:variable name="usr_names" select="/user_manager/usrNames/@name"/>
    ...
    <xsl:template name="main">
        <head>
            ...
            window.onload = function(){
                ...
                document.frmXml.usr_group_lst_single.value=trim(usg_title);
                ...
                document.frmXml.usr_grade_lst_single.value=trim(ugr_title);
    ...
    <xsl:template name="lang_ch">
        <xsl:apply-templates>
            ...
            <xsl:with-param name="lab_user_name">用戶名：</xsl:with-param>
            ...
            <xsl:with-param name="lab_confirm">修改用戶組出錯： 以下用戶為培訓管理員沒有權限分配的系統管理員角色，無法進行修改用戶組，是否繼續執行？</xsl:with-param>
        </xsl:apply-templates>
    </xsl:template>
    <xsl:template name="lang_gb">
        <xsl:apply-templates>
            ...
            <xsl:with-param name="lab_user_name">用户名：</xsl:with-param>
            ...
            <xsl:with-param name="lab_confirm">修改用户组出错： 以下用户为培训管理员没有权限分配的系统管理员角色，无法进行修改用户组，是否继续执行？</xsl:with-param>
    ...
    <xsl:template name="lang_en">
        <xsl:apply-templates>
            ...
            <xsl:with-param name="lab_confirm">Error message: The following user for TA have no legal power distribution SA role, can't modify user groups, whether to continue?</xsl:with-param>
            <xsl:with-param name="lab_user_name">User Id：</xsl:with-param>
        </xsl:apply-templates>
    </xsl:template>
    <!-- =============================================================== -->
    <xsl:template match="/user_manager/user">
        ...
        <table class="Bg" cellspacing="0" cellpadding="3" border="0" width="982" >
            ...
            <tr>
                <td  width="100%"  style="vertical-align:top"  colspan="10">
                    <span class="wzb-form-star"><xsl:value-of select="$lab_confirm"/></span>       
                </td>   
            </tr>
            <tr>
                <td  width="100%"  style="vertical-align:top"  colspan="10">
                  <xsl:value-of select="$lab_user_name"/>  <xsl:value-of select="$usr_names" />
                </td>   
            </tr>
            ...
            <tr>
                <td align="center" valign="middle">
                    <xsl:call-template name="wb_gen_form_button">
                        ...
                        <xsl:with-param name="wb_gen_btn_href">usr.user.upd_batch_exec2(document.frmXml, '<xsl:value-of select="$wb_lang"/>')</xsl:with-param>
                    </xsl:call-template>
