...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="tcr_id" select="/*/header/training_center/@id"/>		
	<!-- =============================================================== -->
	<xsl:template match="/">
		<html>
			<xsl:apply-templates/>
	...
	<xsl:template match="module | fixed_assessment">
		<head>
			...
			<script language="JavaScript"><![CDATA[
				...
				function load_frame(){
					var url = obj.pick_obj_url('QUE', ]]>'<xsl:value-of select="$tcr_id"/>'<![CDATA[)
		...
		str+='<frame name="frame" src="../htm/loading.htm?lang=]]><xsl:value-of select="$wb_lang"/><![CDATA[" marginwidth="0" marginheight="0" scrolling="auto" frameborder="0" noresize="noresize">'
		str+='</frameset>'
		str+='<frame src="../htm/empty.htm" scrolling="NO" noresize/>'
		str+='</frameset>'
		//str+='</textarea>'
